...
#define I40E_TX_OFFLOAD_MASK                                                   \
  (RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_OUTER_IPV6 | RTE_MBUF_F_TX_IPV4 |  \
   RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_L4_MASK |       \
   RTE_MBUF_F_TX_OUTER_IP_CKSUM | RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_QINQ | \
   RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_TUNNEL_MASK |                            \
   RTE_MBUF_F_TX_OUTER_UDP_CKSUM | I40E_TX_IEEE1588_TMST)

#define I40E_TX_OFFLOAD_NOTSUP_MASK                                            \
  (RTE_MBUF_F_TX_OFFLOAD_MASK ^ I40E_TX_OFFLOAD_MASK)

#define I40E_TX_OFFLOAD_SIMPLE_SUP_MASK                                        \
  (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_OUTER_IPV4 |        \
   RTE_MBUF_F_TX_OUTER_IPV6)

#define I40E_TX_OFFLOAD_SIMPLE_NOTSUP_MASK                                     \
  (RTE_MBUF_F_TX_OFFLOAD_MASK ^ I40E_TX_OFFLOAD_SIMPLE_SUP_MASK)

static int i40e_monitor_callback(
...
static inline void i40e_rxd_to_vlan_tci(struct rte_mbuf *mb,
                                        volatile union i40e_rx_desc *rxdp) {
  if (rte_le_to_cpu_64(rxdp->wb.qword1.status_error_len) &
      (1 << I40E_RX_DESC_STATUS_L2TAG1P_SHIFT)) {
    mb->ol_flags |= RTE_MBUF_F_RX_VLAN | RTE_MBUF_F_RX_VLAN_STRIPPED;
    mb->vlan_tci = rte_le_to_cpu_16(rxdp->wb.qword0.lo_dword.l2tag1);
...
static inline uint64_t i40e_rxd_status_to_pkt_flags(uint64_t qword) {
  uint64_t flags;

  /* Check if RSS_HASH */
  flags = (((qword >> I40E_RX_DESC_STATUS_FLTSTAT_SHIFT) &
            I40E_RX_DESC_FLTSTAT_RSS_HASH) == I40E_RX_DESC_FLTSTAT_RSS_HASH)
              ? RTE_MBUF_F_RX_RSS_HASH
              : 0;

  /* Check if FDIR Match */
  flags |=
      (qword & (1 << I40E_RX_DESC_STATUS_FLM_SHIFT) ? RTE_MBUF_F_RX_FDIR : 0);

  return flags;
}

static inline uint64_t i40e_rxd_error_to_pkt_flags(uint64_t qword) {
  uint64_t flags = 0;
 ...
 */
#ifdef RTE_LIBRTE_IEEE1588
static inline uint64_t i40e_get_iee15888_flags(struct rte_mbuf *mb,
                                               uint64_t qword) {
  ...
  if ((mb->packet_type & RTE_PTYPE_L2_MASK) == RTE_PTYPE_L2_ETHER_TIMESYNC)
...
static inline uint64_t i40e_rxd_build_fdir(volatile union i40e_rx_desc *rxdp,
                                           struct rte_mbuf *mb) {
  uint64_t flags = 0;
#ifndef RTE_LIBRTE_I40E_16BYTE_RX_DESC
  ...
  if (flexbh == I40E_RX_DESC_EXT_STATUS_FLEXBH_FD_ID) {
    mb->hash.fdir.hi = rte_le_to_cpu_32(rxdp->wb.qword3.hi_dword.fd_id);
    flags |= RTE_MBUF_F_RX_FDIR_ID;
  } else if (flexbh == I40E_RX_DESC_EXT_STATUS_FLEXBH_FLEX) {
    mb->hash.fdir.hi = rte_le_to_cpu_32(rxdp->wb.qword3.hi_dword.flex_bytes_hi);
    flags |= RTE_MBUF_F_RX_FDIR_FLX;
  }
  if (flexbl == I40E_RX_DESC_EXT_STATUS_FLEXBL_FLEX) {
    mb->hash.fdir.lo = rte_le_to_cpu_32(rxdp->wb.qword3.lo_dword.flex_bytes_lo);
    flags |= RTE_MBUF_F_RX_FDIR_FLX;
  }
#else
...
static inline void i40e_parse_tunneling_params(uint64_t ol_flags,
  ...
  *cd_tunneling |= (tx_offload.outer_l3_len >> 2)
   ...
   * its last Ethertype.
   */
  *cd_tunneling |= (tx_offload.l2_len >> 1) << I40E_TXD_CTX_QW0_NATLEN_SHIFT;
}

static inline void i40e_txd_enable_checksum(uint64_t ol_flags, uint32_t *td_cmd,
                                            uint32_t *td_offset,
                                            union i40e_tx_offload tx_offload) {
  /* Set MACLEN */
  if (!(ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK))
    *td_offset |= (tx_offload.l2_len >> 1) << I40E_TX_DESC_LENGTH_MACLEN_SHIFT;

  /* Enable L3 checksum offloads */
  if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    *td_cmd |= I40E_TX_DESC_CMD_IIPT_IPV4_CSUM;
    *td_offset |= (tx_offload.l3_len >> 2) << I40E_TX_DESC_LENGTH_IPLEN_SHIFT;
...
static inline uint64_t i40e_build_ctob(uint32_t td_cmd, uint32_t td_offset,
                                       unsigned int size, uint32_t td_tag) {
...
static inline int i40e_xmit_cleanup(struct i40e_tx_queue *txq) {
...
#endif
static inline int i40e_rx_scan_hw_ring(struct i40e_rx_queue *rxq) {
  volatile union i40e_rx_desc *rxdp;
   ...
   */
  for (i = 0; i < RTE_PMD_I40E_RX_MAX_BURST;
       i += I40E_LOOK_AHEAD, rxdp += I40E_LOOK_AHEAD, rxep += I40E_LOOK_AHEAD) {
    /* Read desc statuses backwards to avoid race condition */
    for (j = I40E_LOOK_AHEAD - 1; j >= 0; j--) {
      qword1 = rte_le_to_cpu_64(rxdp[j].wb.qword1.status_error_len);
      s[j] = (qword1 & I40E_RXD_QW1_STATUS_MASK) >> I40E_RXD_QW1_STATUS_SHIFT;
    }
    ...
    for (j = 0; j < nb_dd; j++) {
      ...
      mb->packet_type =
          ptype_tbl[(uint8_t)((qword1 & I40E_RXD_QW1_PTYPE_MASK) >>
                              I40E_RXD_QW1_PTYPE_SHIFT)];
      if (pkt_flags & RTE_MBUF_F_RX_RSS_HASH)
        mb->hash.rss = rte_le_to_cpu_32(rxdp[j].wb.qword0.hi_dword.rss);
...
static inline uint16_t i40e_rx_fill_from_stage(struct i40e_rx_queue *rxq,
...
static inline int i40e_rx_alloc_bufs(struct i40e_rx_queue *rxq) {
  volatile union i40e_rx_desc *rxdp;
  struct i40e_rx_entry *rxep;
  struct rte_mbuf *mb;
...
uint16_t i40e_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                        uint16_t nb_pkts) {
  ...
  while (nb_rx < nb_pkts) {
    ...
    qword1 = rte_le_to_cpu_64(rxdp->wb.qword1.status_error_len);
    rx_status =
        (qword1 & I40E_RXD_QW1_STATUS_MASK) >> I40E_RXD_QW1_STATUS_SHIFT;
    ...
    pkt_flags = i40e_rxd_status_to_pkt_flags(qword1);
    pkt_flags |= i40e_rxd_error_to_pkt_flags(qword1);
    rxm->packet_type = ptype_tbl[(uint8_t)((qword1 & I40E_RXD_QW1_PTYPE_MASK) >>
                                           I40E_RXD_QW1_PTYPE_SHIFT)];
    if (pkt_flags & RTE_MBUF_F_RX_RSS_HASH)
...
uint16_t i40e_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                  uint16_t nb_pkts) {
  struct i40e_rx_queue *rxq = rx_queue;
  ...
  while (nb_rx < nb_pkts) {
    ...
    first_seg->packet_type =
        ptype_tbl[(uint8_t)((qword1 & I40E_RXD_QW1_PTYPE_MASK) >>
                            I40E_RXD_QW1_PTYPE_SHIFT)];
    if (pkt_flags & RTE_MBUF_F_RX_RSS_HASH)
      first_seg->hash.rss = rte_le_to_cpu_32(rxd.wb.qword0.hi_dword.rss);
    if (pkt_flags & RTE_MBUF_F_RX_FDIR)
...
static inline uint16_t i40e_calc_context_desc(uint64_t flags) {
  static uint64_t mask = RTE_MBUF_F_TX_OUTER_IP_CKSUM | RTE_MBUF_F_TX_TCP_SEG |
                         RTE_MBUF_F_TX_QINQ | RTE_MBUF_F_TX_TUNNEL_MASK;
...
static inline uint64_t i40e_set_tso_ctx(struct rte_mbuf *mbuf,
                                        union i40e_tx_offload tx_offload) {
  uint64_t ctx_desc = 0;
  ...
  ctx_desc |= ((uint64_t)cd_cmd << I40E_TXD_CTX_QW1_CMD_SHIFT) |
              ((uint64_t)cd_tso_len << I40E_TXD_CTX_QW1_TSO_LEN_SHIFT) |
              ((uint64_t)mbuf->tso_segsz << I40E_TXD_CTX_QW1_MSS_SHIFT);
...
uint16_t i40e_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                        uint16_t nb_pkts) {
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
     ...
     */
    if (ol_flags & RTE_MBUF_F_TX_TCP_SEG)
      nb_used = (uint16_t)(i40e_calc_pkt_desc(tx_pkt) + nb_ctx);
    else
    ...
    if (ol_flags & I40E_TX_CKSUM_OFFLOAD_MASK)
      i40e_txd_enable_checksum(ol_flags, &td_cmd, &td_offset, tx_offload);

    if (nb_ctx) {
      /* Setup TX context descriptor if required */
      volatile struct i40e_tx_context_desc *ctx_txd =
          (volatile struct i40e_tx_context_desc *)&txr[tx_id];
      ...
      if (ol_flags & RTE_MBUF_F_TX_TCP_SEG)
        cd_type_cmd_tso_mss |= i40e_set_tso_ctx(tx_pkt, tx_offload);
      else {
#ifdef RTE_LIBRTE_IEEE1588
      ...
      ctx_txd->type_cmd_tso_mss = rte_cpu_to_le_64(cd_type_cmd_tso_mss);
      ...
      PMD_TX_LOG(DEBUG,
                 ...
                 tx_pkt, tx_id, ctx_txd->tunneling_params, ctx_txd->l2tag2,
                 ctx_txd->rsvd, ctx_txd->type_cmd_tso_mss);
    ...
    do {
      ...
      while ((ol_flags & RTE_MBUF_F_TX_TCP_SEG) &&
        ...
        txd->cmd_type_offset_bsz =
            i40e_build_ctob(td_cmd, td_offset, I40E_MAX_DATA_PER_TXD, td_tag);
      ...
      PMD_TX_LOG(DEBUG,
                 ...
                 "td_len: %u;\n"
                 "td_tag: %#x;\n",
                 tx_pkt, tx_id, buf_dma_addr, td_cmd, td_offset, slen, td_tag);

      txd->buffer_addr = rte_cpu_to_le_64(buf_dma_addr);
      txd->cmd_type_offset_bsz =
          i40e_build_ctob(td_cmd, td_offset, slen, td_tag);
      txe->last_id = tx_last;
      tx_id = txe->next_id;
      txe = txn;
      m_seg = m_seg->next;
    ...
    txd->cmd_type_offset_bsz |=
        rte_cpu_to_le_64(((uint64_t)td_cmd) << I40E_TXD_QW1_CMD_SHIFT);
  }

end_of_tx:
  PMD_TX_LOG(DEBUG, "port_id=%u queue_id=%u tx_tail=%u nb_tx=%u",
             (unsigned)txq->port_id, (unsigned)txq->queue_id, (unsigned)tx_id,
             (unsigned)nb_tx);
...
static __rte_always_inline int i40e_tx_free_bufs(struct i40e_tx_queue *txq) {
...
static inline void tx4(volatile struct i40e_tx_desc *txdp,
  ...
  for (i = 0; i < 4; i++, txdp++, pkts++) {
    ...
    txdp->cmd_type_offset_bsz =
        i40e_build_ctob((uint32_t)I40E_TD_CMD, 0, (*pkts)->data_len, 0);
  }
}

/* Populate 1 descriptor with data from 1 mbuf */
static inline void tx1(volatile struct i40e_tx_desc *txdp,
                       struct rte_mbuf **pkts) {
  uint64_t dma_addr;
...
static inline void i40e_tx_fill_hw_ring(struct i40e_tx_queue *txq,
                                        struct rte_mbuf **pkts,
                                        uint16_t nb_pkts) {
...
static inline uint16_t tx_xmit_pkts(struct i40e_tx_queue *txq,
  ...
  if ((txq->tx_tail + nb_pkts) > txq->nb_tx_desc) {
    ...
    txr[txq->tx_next_rs].cmd_type_offset_bsz |= rte_cpu_to_le_64(
        ((uint64_t)I40E_TX_DESC_CMD_RS) << I40E_TXD_QW1_CMD_SHIFT);
...
static uint16_t i40e_xmit_pkts_simple(void *tx_queue, struct rte_mbuf **tx_pkts,
                                      uint16_t nb_pkts) {
  uint16_t nb_tx = 0;

  if (likely(nb_pkts <= I40E_TX_MAX_BURST))
    return tx_xmit_pkts((struct i40e_tx_queue *)tx_queue, tx_pkts, nb_pkts);

  while (nb_pkts) {
    uint16_t ret, num = (uint16_t)RTE_MIN(nb_pkts, I40E_TX_MAX_BURST);

    ret = tx_xmit_pkts((struct i40e_tx_queue *)tx_queue, &tx_pkts[nb_tx], num);
...
static uint16_t i40e_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
                                   uint16_t nb_pkts) {
  uint16_t nb_tx = 0;
  ...
  while (nb_pkts) {
    ...
    ret = i40e_xmit_fixed_burst_vec(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
...
/* Tx mbuf check */
static uint16_t i40e_xmit_pkts_check(void *tx_queue, struct rte_mbuf **tx_pkts,
                                     uint16_t nb_pkts) {
  ...
  for (idx = 0; idx < nb_pkts; idx++) {
    ...
    if ((adapter->mbuf_check & I40E_MBUF_CHECK_F_TX_SIZE) &&
        (mb->data_len > mb->pkt_len || mb->data_len < I40E_TX_MIN_PKT_LEN ||
 ...
 *
 **********************************************************************/
uint16_t i40e_simple_prep_pkts(__rte_unused void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (m->pkt_len < I40E_TX_MIN_PKT_LEN || m->pkt_len > I40E_FRAME_SIZE_MAX) {
      rte_errno = EINVAL;
      return i;
    }
  }
 ...
 *
 **********************************************************************/
uint16_t i40e_prep_pkts(__rte_unused void *tx_queue, struct rte_mbuf **tx_pkts,
                        uint16_t nb_pkts) {
  int i, ret;
  uint64_t ol_flags;
  struct rte_mbuf *m;
 ...
 * belongs to.
 */
static struct i40e_vsi *i40e_pf_get_vsi_by_qindex(struct i40e_pf *pf,
                                                  uint16_t queue_idx) {
...
static uint16_t i40e_get_queue_offset_by_qindex(struct i40e_pf *pf,
                                                uint16_t queue_idx) {
  /* the queue in MAIN VSI range */
  if (queue_idx < pf->main_vsi->nb_qps)
    return queue_idx;
...
int i40e_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (rxq->rx_deferred_start)
    PMD_DRV_LOG(WARNING, "RX queue %u is deferred start", rx_queue_id);
...
int i40e_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (txq->tx_deferred_start)
    PMD_DRV_LOG(WARNING, "TX queue %u is deferred start", tx_queue_id);
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to switch TX queue %u on", tx_queue_id);
    return err;
  }
...
int i40e_dev_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to switch TX queue %u of", tx_queue_id);
    return err;
  }
...
static int i40e_dev_first_queue(uint16_t idx, void **queues, int num) {
  uint16_t i;
...
static int i40e_dev_rx_queue_setup_runtime(struct rte_eth_dev *dev,
                                           struct i40e_rx_queue *rxq) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  int use_def_burst_func = check_rx_burst_bulk_alloc_preconditions(rxq);
  uint16_t buf_size =
      (uint16_t)(rte_pktmbuf_data_room_size(rxq->mp) - RTE_PKTMBUF_HEADROOM);
  int use_scattered_rx = (rxq->max_pkt_len > buf_size);
  ...
  if (i40e_dev_first_queue(rxq->queue_id, dev->data->rx_queues,
    ...
    PMD_DRV_LOG(ERR,
                "Vector mode is allowed, but descriptor"
...
int i40e_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (nb_desc % I40E_ALIGN_RING_DESC != 0 || (nb_desc > I40E_MAX_RING_DESC) ||
  ...
  rxq = rte_zmalloc_socket("i40e rx queue", sizeof(struct i40e_rx_queue),
  ...
  ring_size = RTE_ALIGN(len * sizeof(union i40e_rx_desc), I40E_DMA_MEM_ALIGN);

  rz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, ring_size,
                                I40E_RING_BASE_ALIGN, socket_id);
  if (!rz) {
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("i40e rx sw ring", sizeof(struct i40e_rx_entry) * len,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
    i40e_rx_queue_release(rxq);
...
int i40e_dev_tx_descriptor_status(void *tx_queue, uint16_t offset) {
  ...
  expect = rte_cpu_to_le_64(I40E_TX_DESC_DTYPE_DESC_DONE
...
static int i40e_dev_tx_queue_setup_runtime(struct rte_eth_dev *dev,
                                           struct i40e_tx_queue *txq) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);

  if (i40e_tx_queue_init(txq) != I40E_SUCCESS) {
    PMD_DRV_LOG(ERR, "Failed to do TX queue initialization");
    return -EINVAL;
  }

  if (i40e_dev_first_queue(txq->queue_id, dev->data->tx_queues,
...
int i40e_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                            uint16_t nb_desc, unsigned int socket_id,
  ...
  txq = rte_zmalloc_socket("i40e tx queue", sizeof(struct i40e_tx_queue),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq) {
    PMD_DRV_LOG(ERR, "Failed to allocate memory for "
                     "tx queue structure");
  ...
  txq->sw_ring = rte_zmalloc_socket("i40e tx sw ring",
...
const struct rte_memzone *i40e_memzone_reserve(const char *name, uint32_t len,
                                               int socket_id) {
  ...
  mz = rte_memzone_reserve_aligned(
      name, len, socket_id, RTE_MEMZONE_IOVA_CONTIG, I40E_RING_BASE_ALIGN);
  return mz;
}
...
static int i40e_tx_done_cleanup_full(struct i40e_tx_queue *txq,
                                     uint32_t free_cnt) {
...
static int i40e_tx_done_cleanup_simple(struct i40e_tx_queue *txq,
                                       uint32_t free_cnt) {
  int i, n, cnt;
...
static int i40e_tx_done_cleanup_vec(struct i40e_tx_queue *txq __rte_unused,
...
void i40e_reset_tx_queue(struct i40e_tx_queue *txq) {
  ...
  for (i = 0; i < txq->nb_tx_desc; i++) {
    ...
    txd->cmd_type_offset_bsz = rte_cpu_to_le_64(I40E_TX_DESC_DTYPE_DESC_DONE);
    txe[i].mbuf = NULL;
    txe[i].last_id = i;
    txe[prev].next_id = i;
    prev = i;
...
int i40e_tx_queue_init(struct i40e_tx_queue *txq) {
  ...
  qtx_ctl |=
      ((hw->pf_id << I40E_QTX_CTL_PF_INDX_SHIFT) & I40E_QTX_CTL_PF_INDX_MASK);
 ...
 */
static int i40e_rx_queue_config(struct i40e_rx_queue *rxq) {
  ...
  switch (pf->flags &
          (I40E_FLAG_HEADER_SPLIT_DISABLED | I40E_FLAG_HEADER_SPLIT_ENABLED)) {
  case I40E_FLAG_HEADER_SPLIT_ENABLED: /* Not supported */
    rxq->rx_hdr_len =
        RTE_ALIGN(I40E_RXBUF_SZ_1024, (1 << I40E_RXQ_CTX_HBUFF_SHIFT));
    rxq->rx_buf_len =
        RTE_ALIGN(I40E_RXBUF_SZ_2048, (1 << I40E_RXQ_CTX_DBUFF_SHIFT));
  ...
  default:
    rxq->rx_hdr_len = 0;
    rxq->rx_buf_len =
        RTE_ALIGN_FLOOR(buf_size, (1 << I40E_RXQ_CTX_DBUFF_SHIFT));
    rxq->rx_buf_len = RTE_MIN(rxq->rx_buf_len, I40E_RX_MAX_DATA_BUF_SIZE);
    rxq->hs_mode = i40e_header_split_none;
    break;
  }

  rxq->max_pkt_len = RTE_MIN(hw->func_caps.rx_buf_chain_len * rxq->rx_buf_len,
  ...
  if (rxq->max_pkt_len < RTE_ETHER_MIN_LEN ||
    ...
    PMD_DRV_LOG(ERR,
                ...
                (uint32_t)RTE_ETHER_MIN_LEN, (uint32_t)I40E_FRAME_SIZE_MAX);
...
enum i40e_status_code i40e_fdir_setup_tx_resources(struct i40e_pf *pf) {
  ...
  /* Allocate the TX queue data structure. */
  txq = rte_zmalloc_socket("i40e fdir tx queue", sizeof(struct i40e_tx_queue),
  ...
  tz = rte_eth_dma_zone_reserve(dev, "fdir_tx_ring", I40E_FDIR_QUEUE_ID,
                                ring_size, I40E_RING_BASE_ALIGN, SOCKET_ID_ANY);
  if (!tz) {
    i40e_tx_queue_release(txq);
    PMD_DRV_LOG(ERR, "Failed to reserve DMA memory for TX.");
    return I40E_ERR_NO_MEMORY;
...
enum i40e_status_code i40e_fdir_setup_rx_resources(struct i40e_pf *pf) {
  ...
  /* Allocate the RX queue data structure. */
  rxq = rte_zmalloc_socket("i40e fdir rx queue", sizeof(struct i40e_rx_queue),
                           RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  if (!rxq) {
    PMD_DRV_LOG(ERR, "Failed to allocate memory for "
                     "rx queue structure.");
  ...
  rz = rte_eth_dma_zone_reserve(dev, "fdir_rx_ring", I40E_FDIR_QUEUE_ID,
                                ring_size, I40E_RING_BASE_ALIGN, SOCKET_ID_ANY);
  if (!rz) {
    i40e_rx_queue_release(rxq);
    PMD_DRV_LOG(ERR, "Failed to reserve DMA memory for RX.");
...
void i40e_recycle_rxq_info_get(
...
void __rte_cold i40e_set_rx_function(struct rte_eth_dev *dev) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  if (ad->rx_vec_allowed &&
    ...
    if (dev->data->scattered_rx) {
      if (ad->rx_use_avx512) {
        ...
        PMD_INIT_LOG(DEBUG, "Using %sVector Scattered Rx (port %d).",
                     ad->rx_use_avx2 ? "avx2 " : "", dev->data->port_id);
        dev->rx_pkt_burst = ad->rx_use_avx2 ? i40e_recv_scattered_pkts_vec_avx2
      ...
      if (ad->rx_use_avx512) {
        ...
        PMD_INIT_LOG(DEBUG, "Using %sVector Rx (port %d).",
                     ad->rx_use_avx2 ? "avx2 " : "", dev->data->port_id);
        dev->rx_pkt_burst =
            ad->rx_use_avx2 ? i40e_recv_pkts_vec_avx2 : i40e_recv_pkts_vec;
        dev->recycle_rx_descriptors_refill =
            i40e_recycle_rx_descriptors_refill_vec;
      }
    }
    ...
    if (dev->data->scattered_rx) {
      PMD_INIT_LOG(DEBUG, "Using Vector Scattered Rx (port %d).",
                   dev->data->port_id);
      dev->rx_pkt_burst = i40e_recv_scattered_pkts_vec;
    } else {
      PMD_INIT_LOG(DEBUG, "Using Vector Rx (port %d).", dev->data->port_id);
      dev->rx_pkt_burst = i40e_recv_pkts_vec;
    }
#endif /* RTE_ARCH_X86 */
    ...
    PMD_INIT_LOG(DEBUG, "Simple Rx path will be used on port=%d.",
                 dev->data->port_id);
    dev->rx_pkt_burst =
        dev->data->scattered_rx ? i40e_recv_scattered_pkts : i40e_recv_pkts;
  }

  /* Propagate information about RX function choice through all queues. */
  if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
    rx_using_sse = (dev->rx_pkt_burst == i40e_recv_scattered_pkts_vec ||
                    dev->rx_pkt_burst == i40e_recv_pkts_vec ||
#ifdef CC_AVX512_SUPPORT
                    dev->rx_pkt_burst == i40e_recv_scattered_pkts_vec_avx512 ||
                    dev->rx_pkt_burst == i40e_recv_pkts_vec_avx512 ||
...
void __rte_cold i40e_set_tx_function_flag(struct rte_eth_dev *dev,
                                          struct i40e_tx_queue *txq) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  if (ad->tx_vec_allowed)
    ...
    PMD_INIT_LOG(DEBUG, "Neither simple nor vector Tx enabled on Tx queue %u\n",
                 txq->queue_id);
}

void __rte_cold i40e_set_tx_function(struct rte_eth_dev *dev) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  if (ad->tx_simple_allowed) {
    if (ad->tx_vec_allowed &&
      ...
      if (ad->tx_use_avx512) {
        ...
        dev->tx_pkt_burst =
            ad->tx_use_avx2 ? i40e_xmit_pkts_vec_avx2 : i40e_xmit_pkts_vec;
        dev->recycle_tx_mbufs_reuse = i40e_recycle_tx_mbufs_reuse_vec;
      }
#else  /* RTE_ARCH_X86 */
      PMD_INIT_LOG(DEBUG, "Using Vector Tx (port %d).", dev->data->port_id);
      dev->tx_pkt_burst = i40e_xmit_pkts_vec;
      dev->recycle_tx_mbufs_reuse = i40e_recycle_tx_mbufs_reuse_vec;
#endif /* RTE_ARCH_X86 */
    } else {
...
int i40e_tx_burst_mode_get(struct rte_eth_dev *dev,
                           __rte_unused uint16_t queue_id,
                           struct rte_eth_burst_mode *mode) {
...
void __rte_cold i40e_set_default_ptype_table(struct rte_eth_dev *dev) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  int i;
...
void __rte_cold i40e_set_default_pctype_table(struct rte_eth_dev *dev) {
  struct i40e_adapter *ad = I40E_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int i;
  ...
  if (hw->mac.type == I40E_MAC_X722 || hw->mac.type == I40E_MAC_X722_VF) {
    ad->pctypes_tbl[RTE_ETH_FLOW_NONFRAG_IPV4_UDP] |=
...
#ifndef RTE_ARCH_X86
uint16_t i40e_recv_pkts_vec_avx2(void __rte_unused *rx_queue,
                                 struct rte_mbuf __rte_unused **rx_pkts,
                                 uint16_t __rte_unused nb_pkts) {
  return 0;
}
...
uint16_t i40e_xmit_pkts_vec_avx2(void __rte_unused *tx_queue,
