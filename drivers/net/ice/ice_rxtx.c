...
#define ICE_TX_CKSUM_OFFLOAD_MASK                                              \
  (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_TCP_SEG |    \
...
static int ice_monitor_callback(
...
static inline uint8_t ice_proto_xtr_type_to_rxdid(uint8_t xtr_type) {
  ...
  return xtr_type < RTE_DIM(rxdid_map) ? rxdid_map[xtr_type]
                                       : ICE_RXDID_COMMS_OVS;
}
...
static enum ice_status ice_program_hw_rx_queue(struct ice_rx_queue *rxq) {
  ...
  rxq->max_pkt_len =
      RTE_MIN((uint32_t)ICE_SUPPORT_CHAIN_NUM * rxq->rx_buf_len, frame_size);

  if (rxq->max_pkt_len <= RTE_ETHER_MIN_LEN ||
      rxq->max_pkt_len > ICE_FRAME_SIZE_MAX) {
    PMD_DRV_LOG(ERR,
                "maximum packet length must "
                "be larger than %u and smaller than %u",
                (uint32_t)RTE_ETHER_MIN_LEN, (uint32_t)ICE_FRAME_SIZE_MAX);
    return -EINVAL;
  }

  if (!rxq->ts_enable && (rxq->offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP)) {
    /* Register mbuf field and flag for Rx timestamp */
    err = rte_mbuf_dyn_rx_timestamp_register(&ice_timestamp_dynfield_offset,
  ...
  if (!(pf->supported_rxdid & BIT(rxdid))) {
    PMD_DRV_LOG(ERR, "currently package doesn't support RXDID (%u)", rxdid);
    return -EINVAL;
  }
   ...
   */
  regval = (rxdid << QRXFLXP_CNTXT_RXDID_IDX_S) & QRXFLXP_CNTXT_RXDID_IDX_M;

  /* increasing context priority to pick up profile ID;
   * default is 0x01; setting to 0x03 to ensure profile
   * is programming if prev context is of same priority
   */
  regval |= (0x03 << QRXFLXP_CNTXT_RXDID_PRIO_S) & QRXFLXP_CNTXT_RXDID_PRIO_M;
...
static int ice_alloc_rx_queue_mbufs(struct ice_rx_queue *rxq) {
...
static void _ice_rx_queue_release_mbufs(struct ice_rx_queue *rxq) {
  uint16_t i;
 ...
 */
static int ice_switch_rx_queue(struct ice_hw *hw, uint16_t q_idx, bool on) {
  uint32_t reg;
  uint16_t j;
  ...
  for (j = 0; j < ICE_CHK_Q_ENA_COUNT; j++) {
    ...
    reg = ICE_READ_REG(hw, QRX_CTRL(q_idx));
    if (on) {
      if ((reg & QRX_CTRL_QENA_REQ_M) && (reg & QRX_CTRL_QENA_STAT_M))
        break;
    } else {
      if (!(reg & QRX_CTRL_QENA_REQ_M) && !(reg & QRX_CTRL_QENA_STAT_M))
        break;
  ...
  if (j >= ICE_CHK_Q_ENA_COUNT) {
    PMD_DRV_LOG(ERR, "Failed to %s rx queue[%u]", (on ? "enable" : "disable"),
                q_idx);
    return -ETIMEDOUT;
  }
...
static void ice_reset_rx_queue(struct ice_rx_queue *rxq) {
  unsigned int i;
  uint16_t len;
...
int ice_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (rx_queue_id >= dev->data->nb_rx_queues) {
    PMD_DRV_LOG(ERR, "RX queue %u is out of range %u", rx_queue_id,
  ...
  if (dev->data->rx_queue_state[rx_queue_id] == RTE_ETH_QUEUE_STATE_STARTED)
    return 0;
  ...
  if (err) {
    PMD_DRV_LOG(ERR, "fail to program RX queue %u", rx_queue_id);
    return -EIO;
  }
...
int ice_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (rx_queue_id < dev->data->nb_rx_queues) {
    ...
    if (err) {
      PMD_DRV_LOG(ERR, "Failed to switch RX queue %u off", rx_queue_id);
      return -EINVAL;
    }
    rxq->rx_rel_mbufs(rxq);
    ice_reset_rx_queue(rxq);
...
int ice_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (tx_queue_id >= dev->data->nb_tx_queues) {
    PMD_DRV_LOG(ERR, "TX queue %u is out of range %u", tx_queue_id,
  ...
  if (!txq || !txq->q_set) {
    PMD_DRV_LOG(ERR, "TX queue %u is not available or setup", tx_queue_id);
    return -EINVAL;
  }

  if (dev->data->tx_queue_state[tx_queue_id] == RTE_ETH_QUEUE_STATE_STARTED)
    return 0;
  ...
  err = ice_ena_vsi_txq(hw->port_info, vsi->idx, 0, tx_queue_id, 1, txq_elem,
                        buf_len, NULL);
  if (err) {
...
static enum ice_status ice_fdir_program_hw_rx_queue(struct ice_rx_queue *rxq) {
  struct ice_vsi *vsi = rxq->vsi;
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
...
int ice_fdir_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  rxq = pf->fdir.rxq;
  if (!rxq || !rxq->q_set) {
    PMD_DRV_LOG(ERR, "FDIR RX queue %u not available or setup", rx_queue_id);
    return -EINVAL;
  }

  err = ice_fdir_program_hw_rx_queue(rxq);
  if (err) {
    PMD_DRV_LOG(ERR, "fail to program FDIR RX queue %u", rx_queue_id);
    return -EIO;
  }
  ...
  err = ice_switch_rx_queue(hw, rxq->reg_idx, true);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to switch FDIR RX queue %u on", rx_queue_id);
...
int ice_fdir_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  txq = pf->fdir.txq;
  if (!txq || !txq->q_set) {
    PMD_DRV_LOG(ERR, "FDIR TX queue %u is not available or setup", tx_queue_id);
    return -EINVAL;
  }
...
static void _ice_tx_queue_release_mbufs(struct ice_tx_queue *txq) {
  uint16_t i;
...
static void ice_reset_tx_queue(struct ice_tx_queue *txq) {
  ...
  for (i = 0; i < txq->nb_tx_desc; i++) {
    ...
    txd->cmd_type_offset_bsz = rte_cpu_to_le_64(ICE_TX_DESC_DTYPE_DESC_DONE);
    txe[i].mbuf = NULL;
    txe[i].last_id = i;
    txe[prev].next_id = i;
    prev = i;
...
int ice_tx_queue_stop(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  txq = dev->data->tx_queues[tx_queue_id];
  if (!txq) {
    PMD_DRV_LOG(ERR, "TX queue %u is not available", tx_queue_id);
    return -EINVAL;
  }
  ...
  /* Fix me, we assume TC always 0 here */
  status = ice_dis_vsi_txq(hw->port_info, vsi->idx, 0, 1, &q_handle, q_ids,
                           q_teids, ICE_NO_RESET, 0, NULL);
...
int ice_fdir_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  err = ice_switch_rx_queue(hw, rxq->reg_idx, false);
  if (err) {
    PMD_DRV_LOG(ERR, "Failed to switch FDIR RX queue %u off", rx_queue_id);
    return -EINVAL;
  }
  rxq->rx_rel_mbufs(rxq);
...
int ice_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (nb_desc % ICE_ALIGN_RING_DESC != 0 || nb_desc > ICE_MAX_RING_DESC ||
  ...
  rxq = rte_zmalloc_socket(NULL, sizeof(struct ice_rx_queue),
  ...
  rxq->proto_xtr =
      pf->proto_xtr != NULL ? pf->proto_xtr[queue_idx] : PROTO_XTR_NONE;
  ...
  rz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_idx, ring_size,
                                ICE_RING_BASE_ALIGN, socket_id);
  if (!rz) {
  ...
  rxq->sw_ring = rte_zmalloc_socket(NULL, sizeof(struct ice_rx_entry) * len,
                                    RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
    ice_rx_queue_release(rxq);
...
int ice_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                       uint16_t nb_desc, unsigned int socket_id,
  ...
  tx_rs_thresh = (ICE_DEFAULT_TX_RSBIT_THRESH + tx_free_thresh > nb_desc)
                     ? nb_desc - tx_free_thresh
                     : ICE_DEFAULT_TX_RSBIT_THRESH;
  if (tx_conf->tx_rs_thresh)
    tx_rs_thresh = tx_conf->tx_rs_thresh;
  if (tx_rs_thresh + tx_free_thresh > nb_desc) {
    PMD_INIT_LOG(ERR,
                 "tx_rs_thresh + tx_free_thresh must not "
                 "exceed nb_desc. (tx_rs_thresh=%u "
                 "tx_free_thresh=%u nb_desc=%u port = %d queue=%d)",
                 (unsigned int)tx_rs_thresh, (unsigned int)tx_free_thresh,
                 (unsigned int)nb_desc, (int)dev->data->port_id,
                 (int)queue_idx);
    return -EINVAL;
  }
  ...
  if (tx_rs_thresh > tx_free_thresh) {
    PMD_INIT_LOG(ERR,
                 ...
                 " tx_rs_thresh=%u port=%d queue=%d)",
                 (unsigned int)tx_free_thresh, (unsigned int)tx_rs_thresh,
  ...
  txq = rte_zmalloc_socket(NULL, sizeof(struct ice_tx_queue),
                           RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory for "
                      "tx queue structure");
  ...
  ring_size = RTE_ALIGN(ring_size, ICE_DMA_MEM_ALIGN);
  tz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_idx, ring_size,
                                ICE_RING_BASE_ALIGN, socket_id);
  if (!tz) {
  ...
  txq->sw_ring = rte_zmalloc_socket(NULL, sizeof(struct ice_tx_entry) * nb_desc,
                                    RTE_CACHE_LINE_SIZE, socket_id);
  if (!txq->sw_ring) {
    ice_tx_queue_release(txq);
...
#define ICE_RXQ_SCAN_INTERVAL 4
  ...
  while ((desc < rxq->nb_rx_desc) && rte_le_to_cpu_16(rxdp->wb.status_error0) &
...
/* Rx L3/L4 checksum */
static inline uint64_t ice_rxd_error_to_pkt_flags(uint16_t stat_err0) {
  uint64_t flags = 0;
...
static inline void ice_rxd_to_vlan_tci(struct rte_mbuf *mb,
                                       volatile union ice_rx_flex_desc *rxdp) {
...
#endif
  PMD_RX_LOG(DEBUG, "Mbuf vlan_tci: %u, vlan_tci_outer: %u", mb->vlan_tci,
             mb->vlan_tci_outer);
}
...
static inline int ice_rx_scan_hw_ring(struct ice_rx_queue *rxq) {
  volatile union ice_rx_flex_desc *rxdp;
   ...
   */
  for (i = 0; i < ICE_RX_MAX_BURST;
       i += ICE_LOOK_AHEAD, rxdp += ICE_LOOK_AHEAD, rxep += ICE_LOOK_AHEAD) {
    ...
    for (j = 0; j < nb_dd; j++) {
      mb = rxep[j].mbuf;
      pkt_len =
          (rte_le_to_cpu_16(rxdp[j].wb.pkt_len) & ICE_RX_FLX_DESC_PKT_LEN_M) -
          rxq->crc_len;
      mb->data_len = pkt_len;
      mb->pkt_len = pkt_len;
      ...
      if (ad->ptp_ena && ((mb->packet_type & RTE_PTYPE_L2_MASK) ==
                          RTE_PTYPE_L2_ETHER_TIMESYNC)) {
        rxq->time_high = rte_le_to_cpu_32(rxdp[j].wb.flex_ts.ts_high);
        mb->timesync = rxq->queue_id;
        pkt_flags |= RTE_MBUF_F_RX_IEEE1588_PTP;
        if (rxdp[j].wb.time_stamp_low & ICE_PTP_TS_VALID)
...
static inline uint16_t ice_rx_fill_from_stage(struct ice_rx_queue *rxq,
...
static inline int ice_rx_alloc_bufs(struct ice_rx_queue *rxq) {
  volatile union ice_rx_flex_desc *rxdp;
  struct ice_rx_entry *rxep;
  struct rte_mbuf *mb;
  ...
  if (rxq->offloads & RTE_ETH_RX_OFFLOAD_BUFFER_SPLIT) {
    diag_pay = rte_mempool_get_bulk(rxq->rxseg[1].mp, (void *)mbufs_pay,
                                    rxq->rx_free_thresh);
  ...
  rxq->rx_free_trigger = (uint16_t)(rxq->rx_free_trigger + rxq->rx_free_thresh);
...
static inline uint16_t rx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
                                    uint16_t nb_pkts) {
...
static uint16_t ice_recv_scattered_pkts(void *rx_queue,
  ...
  while (nb_rx < nb_pkts) {
    ...
    if (!first_seg) {
      ...
      first_seg->pkt_len = (uint16_t)(first_seg->pkt_len + rx_packet_len);
      first_seg->nb_segs++;
      last_seg->next = rxm;
    }
    ...
    if (unlikely(rxq->crc_len > 0)) {
      ...
      if (rx_packet_len <= RTE_ETHER_CRC_LEN) {
        ...
        rxm->data_len = (uint16_t)(rx_packet_len - RTE_ETHER_CRC_LEN);
    ...
    if (ice_timestamp_dynflag > 0 &&
      ...
      rxq->hw_time_update =
          rte_get_timer_cycles() / (rte_get_timer_hz() / 1000);
      *RTE_MBUF_DYNFIELD(first_seg, (ice_timestamp_dynfield_offset),
                         rte_mbuf_timestamp_t *) = ts_ns;
      pkt_flags |= ice_timestamp_dynflag;
    }
...
int ice_tx_descriptor_status(void *tx_queue, uint16_t offset) {
  ...
  desc =
      ((desc + txq->tx_rs_thresh - 1) / txq->tx_rs_thresh) * txq->tx_rs_thresh;
  ...
  expect = rte_cpu_to_le_64(ICE_TX_DESC_DTYPE_DESC_DONE << ICE_TXD_QW1_DTYPE_S);
...
int ice_fdir_setup_tx_resources(struct ice_pf *pf) {
  ...
  /* Allocate the TX queue data structure. */
  txq = rte_zmalloc_socket("ice fdir tx queue", sizeof(struct ice_tx_queue),
  ...
  tz = rte_eth_dma_zone_reserve(dev, "fdir_tx_ring", ICE_FDIR_QUEUE_ID,
                                ring_size, ICE_RING_BASE_ALIGN, SOCKET_ID_ANY);
  if (!tz) {
    ice_tx_queue_release(txq);
...
int ice_fdir_setup_rx_resources(struct ice_pf *pf) {
  ...
  /* Allocate the RX queue data structure. */
  rxq = rte_zmalloc_socket("ice fdir rx queue", sizeof(struct ice_rx_queue),
                           RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
  if (!rxq) {
    PMD_DRV_LOG(ERR, "Failed to allocate memory for "
                     "rx queue structure.");
  ...
  rz = rte_eth_dma_zone_reserve(dev, "fdir_rx_ring", ICE_FDIR_QUEUE_ID,
                                ring_size, ICE_RING_BASE_ALIGN, SOCKET_ID_ANY);
  if (!rz) {
    ice_rx_queue_release(rxq);
  ...
  rxq->rx_ring_dma = rz->iova;
  memset(rz->addr, 0, ICE_FDIR_NUM_RX_DESC * sizeof(union ice_32byte_rx_desc));
...
static inline void ice_parse_tunneling_params(uint64_t ol_flags,
  ...
  *cd_tunneling |= (tx_offload.outer_l3_len >> 2) << ICE_TXD_CTX_QW0_EIPLEN_S;
   ...
   */
  *cd_tunneling |= (tx_offload.l2_len >> 1) << ICE_TXD_CTX_QW0_NATLEN_S;
...
static inline void ice_txd_enable_checksum(uint64_t ol_flags, uint32_t *td_cmd,
                                           uint32_t *td_offset,
                                           union ice_tx_offload tx_offload) {
  /* Set MACLEN */
  if (!(ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK))
    *td_offset |= (tx_offload.l2_len >> 1) << ICE_TX_DESC_LEN_MACLEN_S;

  /* Enable L3 checksum offloads */
  if (ol_flags & RTE_MBUF_F_TX_IP_CKSUM) {
    *td_cmd |= ICE_TX_DESC_CMD_IIPT_IPV4_CSUM;
    *td_offset |= (tx_offload.l3_len >> 2) << ICE_TX_DESC_LEN_IPLEN_S;
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    *td_offset |= (tx_offload.l4_len >> 2) << ICE_TX_DESC_LEN_L4_LEN_S;
    return;
  }
  ...
  case RTE_MBUF_F_TX_TCP_CKSUM:
    *td_cmd |= ICE_TX_DESC_CMD_L4T_EOFT_TCP;
    *td_offset |= (sizeof(struct rte_tcp_hdr) >> 2) << ICE_TX_DESC_LEN_L4_LEN_S;
    break;
  ...
  case RTE_MBUF_F_TX_UDP_CKSUM:
    *td_cmd |= ICE_TX_DESC_CMD_L4T_EOFT_UDP;
    *td_offset |= (sizeof(struct rte_udp_hdr) >> 2) << ICE_TX_DESC_LEN_L4_LEN_S;
    break;
  default:
    break;
  }
...
static inline int ice_xmit_cleanup(struct ice_tx_queue *txq) {
...
static inline uint64_t ice_build_ctob(uint32_t td_cmd, uint32_t td_offset,
                                      uint16_t size, uint32_t td_tag) {
...
static inline uint16_t ice_calc_context_desc(uint64_t flags) {
  static uint64_t mask = RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG |
                         RTE_MBUF_F_TX_QINQ | RTE_MBUF_F_TX_OUTER_IP_CKSUM |
...
static inline uint64_t ice_set_tso_ctx(struct rte_mbuf *mbuf,
                                       union ice_tx_offload tx_offload) {
  uint64_t ctx_desc = 0;
...
static inline uint16_t ice_calc_pkt_desc(struct rte_mbuf *tx_pkt) {
...
uint16_t ice_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG))
      nb_used = (uint16_t)(ice_calc_pkt_desc(tx_pkt) + nb_ctx);
    else
    ...
    if (ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK) {
      td_offset |= (tx_offload.outer_l2_len >> 1) << ICE_TX_DESC_LEN_MACLEN_S;
      ice_parse_tunneling_params(ol_flags, tx_offload, &cd_tunneling_params);
    }

    /* Enable checksum offloading */
    if (ol_flags & ICE_TX_CKSUM_OFFLOAD_MASK)
      ice_txd_enable_checksum(ol_flags, &td_cmd, &td_offset, tx_offload);

    if (nb_ctx) {
      /* Setup TX context descriptor if required */
      volatile struct ice_tx_ctx_desc *ctx_txd =
          (volatile struct ice_tx_ctx_desc *)&tx_ring[tx_id];
      ...
      if (ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG))
        cd_type_cmd_tso_mss |= ice_set_tso_ctx(tx_pkt, tx_offload);
      else if (ol_flags & RTE_MBUF_F_TX_IEEE1588_TMST)
        cd_type_cmd_tso_mss |=
            ((uint64_t)ICE_TX_CTX_DESC_TSYN << ICE_TXD_CTX_QW1_CMD_S);

      ctx_txd->tunneling_params = rte_cpu_to_le_32(cd_tunneling_params);

      /* TX context descriptor based double VLAN insert */
      if (ol_flags & RTE_MBUF_F_TX_QINQ) {
        cd_l2tag2 = tx_pkt->vlan_tci_outer;
        cd_type_cmd_tso_mss |=
            ((uint64_t)ICE_TX_CTX_DESC_IL2TAG2 << ICE_TXD_CTX_QW1_CMD_S);
      }
      ctx_txd->l2tag2 = rte_cpu_to_le_16(cd_l2tag2);
      ctx_txd->qw1 = rte_cpu_to_le_64(cd_type_cmd_tso_mss);
    ...
    do {
      ...
      while ((ol_flags & (RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_UDP_SEG)) &&
        ...
        txd->cmd_type_offset_bsz = rte_cpu_to_le_64(
            ICE_TX_DESC_DTYPE_DATA | ((uint64_t)td_cmd << ICE_TXD_QW1_CMD_S) |
            ((uint64_t)td_offset << ICE_TXD_QW1_OFFSET_S) |
            ((uint64_t)ICE_MAX_DATA_PER_TXD << ICE_TXD_QW1_TX_BUF_SZ_S) |
            ((uint64_t)td_tag << ICE_TXD_QW1_L2TAG1_S));
    ...
    txd->cmd_type_offset_bsz |=
        rte_cpu_to_le_64(((uint64_t)td_cmd) << ICE_TXD_QW1_CMD_S);
  }
end_of_tx:
...
static __rte_always_inline int ice_tx_free_bufs(struct ice_tx_queue *txq) {
...
static int ice_tx_done_cleanup_full(struct ice_tx_queue *txq,
                                    uint32_t free_cnt) {
  ...
  for (pkt_cnt = 0; pkt_cnt < free_cnt;) {
    ...
    if (txq->tx_rs_thresh > txq->nb_tx_desc - txq->nb_tx_free ||
        tx_id == tx_last)
      break;
...
#ifdef RTE_ARCH_X86
static int ice_tx_done_cleanup_vec(struct ice_tx_queue *txq __rte_unused,
                                   uint32_t free_cnt __rte_unused) {
  return -ENOTSUP;
}
#endif

static int ice_tx_done_cleanup_simple(struct ice_tx_queue *txq,
                                      uint32_t free_cnt) {
  int i, n, cnt;
...
static inline void tx4(volatile struct ice_tx_desc *txdp,
  ...
  for (i = 0; i < 4; i++, txdp++, pkts++) {
    ...
    txdp->cmd_type_offset_bsz =
        ice_build_ctob((uint32_t)ICE_TD_CMD, 0, (*pkts)->data_len, 0);
  }
}

/* Populate 1 descriptor with data from 1 mbuf */
static inline void tx1(volatile struct ice_tx_desc *txdp,
                       struct rte_mbuf **pkts) {
  uint64_t dma_addr;
...
static inline void ice_tx_fill_hw_ring(struct ice_tx_queue *txq,
                                       struct rte_mbuf **pkts,
                                       uint16_t nb_pkts) {
...
static inline uint16_t tx_xmit_pkts(struct ice_tx_queue *txq,
  ...
  if ((txq->tx_tail + nb_pkts) > txq->nb_tx_desc) {
    ...
    txr[txq->tx_next_rs].cmd_type_offset_bsz |=
        rte_cpu_to_le_64(((uint64_t)ICE_TX_DESC_CMD_RS) << ICE_TXD_QW1_CMD_S);
    txq->tx_next_rs = (uint16_t)(txq->tx_rs_thresh - 1);
    txq->tx_tail = 0;
  }
...
static uint16_t ice_xmit_pkts_simple(void *tx_queue, struct rte_mbuf **tx_pkts,
                                     uint16_t nb_pkts) {
  uint16_t nb_tx = 0;

  if (likely(nb_pkts <= ICE_TX_MAX_BURST))
    return tx_xmit_pkts((struct ice_tx_queue *)tx_queue, tx_pkts, nb_pkts);

  while (nb_pkts) {
    uint16_t ret, num = (uint16_t)RTE_MIN(nb_pkts, ICE_TX_MAX_BURST);

    ret = tx_xmit_pkts((struct ice_tx_queue *)tx_queue, &tx_pkts[nb_tx], num);
...
void __rte_cold ice_set_rx_function(struct rte_eth_dev *dev) {
  PMD_INIT_FUNC_TRACE();
  struct ice_adapter *ad = ICE_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
#ifdef RTE_ARCH_X86
  struct ice_rx_queue *rxq;
...
#else
        PMD_DRV_LOG(NOTICE, "AVX512 is not supported in build env");
#endif
      if (!ad->rx_use_avx512 &&
    ...
    if (dev->data->scattered_rx) {
      if (ad->rx_use_avx512) {
        ...
        if (ad->rx_vec_offload_support) {
          ...
          PMD_DRV_LOG(NOTICE, "Using AVX512 Vector Scattered Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_scattered_pkts_vec_avx512;
        }
#endif
        ...
        if (ad->rx_vec_offload_support) {
          ...
          PMD_DRV_LOG(NOTICE, "Using AVX2 Vector Scattered Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_scattered_pkts_vec_avx2;
        }
      } else {
        PMD_DRV_LOG(DEBUG, "Using Vector Scattered Rx (port %d).",
                    dev->data->port_id);
        dev->rx_pkt_burst = ice_recv_scattered_pkts_vec;
      }
    } else {
      if (ad->rx_use_avx512) {
#ifdef CC_AVX512_SUPPORT
        if (ad->rx_vec_offload_support) {
          PMD_DRV_LOG(NOTICE, "Using AVX512 OFFLOAD Vector Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_pkts_vec_avx512_offload;
        } else {
          PMD_DRV_LOG(NOTICE, "Using AVX512 Vector Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_pkts_vec_avx512;
        }
#endif
        ...
        if (ad->rx_vec_offload_support) {
          PMD_DRV_LOG(NOTICE, "Using AVX2 OFFLOAD Vector Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_pkts_vec_avx2_offload;
        } else {
          PMD_DRV_LOG(NOTICE, "Using AVX2 Vector Rx (port %d).",
                      dev->data->port_id);
          dev->rx_pkt_burst = ice_recv_pkts_vec_avx2;
        }
      } else {
        PMD_DRV_LOG(DEBUG, "Using Vector Rx (port %d).", dev->data->port_id);
  ...
  if (dev->data->scattered_rx) {
    ...
    PMD_INIT_LOG(DEBUG, "Using a Scattered function on port %d.",
                 dev->data->port_id);
    dev->rx_pkt_burst = ice_recv_scattered_pkts;
...
int ice_rx_burst_mode_get(struct rte_eth_dev *dev,
...
void __rte_cold ice_set_tx_function_flag(struct rte_eth_dev *dev,
                                         struct ice_tx_queue *txq) {
  struct ice_adapter *ad = ICE_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  if (ad->tx_simple_allowed)
    ...
    PMD_INIT_LOG(DEBUG, "Simple Tx can NOT be enabled on Tx queue %u.",
                 txq->queue_id);
}
...
static inline uint16_t ice_check_empty_mbuf(struct rte_mbuf *tx_pkt) {
  struct rte_mbuf *txd = tx_pkt;
...
/* Tx mbuf check */
static uint16_t ice_xmit_pkts_check(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  struct ice_tx_queue *txq = tx_queue;
  uint16_t idx;
  struct rte_mbuf *mb;
  ...
  for (idx = 0; idx < nb_pkts; idx++) {
    ...
    if ((adapter->devargs.mbuf_check & ICE_MBUF_CHECK_F_TX_SIZE) &&
        (mb->data_len > mb->pkt_len || mb->data_len < ICE_TX_MIN_PKT_LEN ||
         mb->data_len > ICE_FRAME_SIZE_MAX)) {
      PMD_TX_LOG(ERR,
                 "INVALID mbuf: data_len (%u) is out of range, reasonable "
                 "range (%d - %d)\n",
    ...
    if (adapter->devargs.mbuf_check & ICE_MBUF_CHECK_F_TX_SEGMENT) {
      if (!(ol_flags & RTE_MBUF_F_TX_TCP_SEG)) {
        ...
        if (mb->nb_segs > ICE_TX_MTU_SEG_MAX) {
          PMD_TX_LOG(ERR,
                     "INVALID mbuf: nb_segs (%d) exceeds HW limit, maximum "
                     "allowed value is %d\n",
        ...
        if (mb->pkt_len > ICE_FRAME_SIZE_MAX) {
          PMD_TX_LOG(ERR,
                     "INVALID mbuf: pkt_len (%d) exceeds HW limit, maximum "
                     "allowed value is %d\n",
        ...
        if (mb->tso_segsz < ICE_MIN_TSO_MSS ||
           ...
           */
          PMD_TX_LOG(ERR,
                     "INVALID mbuf: tso_segsz (%u) is out of range, reasonable "
                     "range (%d - %u)\n",
...
uint16_t ice_prep_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                       uint16_t nb_pkts) {
  int i, ret;
  uint64_t ol_flags;
  struct rte_mbuf *m;
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (!(ol_flags & RTE_MBUF_F_TX_TCP_SEG) &&
         ...
         * the limites.
         */
        (m->nb_segs > ICE_TX_MTU_SEG_MAX || m->pkt_len > ICE_FRAME_SIZE_MAX)) {
      rte_errno = EINVAL;
      return i;
    } else if (ol_flags & RTE_MBUF_F_TX_TCP_SEG &&
               /** TSO case: tso_segsz, nb_segs, pkt_len not exceed
               ...
               (m->tso_segsz < ICE_MIN_TSO_MSS ||
                ...
                m->nb_segs > ((struct ice_tx_queue *)tx_queue)->nb_tx_desc ||
...
void __rte_cold ice_set_tx_function(struct rte_eth_dev *dev) {
  struct ice_adapter *ad = ICE_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  if (ad->tx_vec_allowed) {
    ...
    if (ad->tx_use_avx512) {
      ...
      if (tx_check_ret == ICE_VECTOR_OFFLOAD_PATH) {
        PMD_DRV_LOG(NOTICE, "Using AVX512 OFFLOAD Vector Tx (port %d).",
                    dev->data->port_id);
      ...
      if (tx_check_ret == ICE_VECTOR_OFFLOAD_PATH) {
        PMD_DRV_LOG(NOTICE, "Using AVX2 OFFLOAD Vector Tx (port %d).",
                    dev->data->port_id);
        dev->tx_pkt_burst = ice_xmit_pkts_vec_avx2_offload;
        dev->tx_pkt_prepare = ice_prep_pkts;
      } else {
        PMD_DRV_LOG(DEBUG, "Using %sVector Tx (port %d).",
                    ad->tx_use_avx2 ? "avx2 " : "", dev->data->port_id);
        dev->tx_pkt_burst =
            ad->tx_use_avx2 ? ice_xmit_pkts_vec_avx2 : ice_xmit_pkts_vec;
      }
    }
...
int ice_tx_burst_mode_get(struct rte_eth_dev *dev,
                          __rte_unused uint16_t queue_id,
                          struct rte_eth_burst_mode *mode) {
 ...
 */
static inline uint32_t ice_get_default_pkt_type(uint16_t ptype) {
  static const uint32_t type_table[ICE_MAX_PKT_TYPE] __rte_cache_aligned = {
      /* L2 types */
      /* [0] reserved */
      ...
      [44] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
             RTE_PTYPE_TUNNEL_GRENAT | RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN |
             RTE_PTYPE_INNER_L4_FRAG,
      ...
      /* PPPoE --> IPv4 */
      [302] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_FRAG,
      [303] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_NONFRAG,
      [304] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_UDP,
      [305] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_TCP,
      [306] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_SCTP,
      [307] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_L4_ICMP,

      /* PPPoE --> IPv6 */
      [308] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_FRAG,
      [309] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_NONFRAG,
      [310] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_UDP,
      [311] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_TCP,
      [312] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_SCTP,
      [313] = RTE_PTYPE_L2_ETHER_PPPOE | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_L4_ICMP,
      /* [314] - [324] reserved */
      ...
      [331] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_TUNNEL_GTPU | RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_INNER_L4_FRAG,
      ...
      [341] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_TUNNEL_GTPU | RTE_PTYPE_INNER_L3_IPV6_EXT_UNKNOWN |
              RTE_PTYPE_INNER_L4_FRAG,
...
void __rte_cold ice_set_default_ptype_table(struct rte_eth_dev *dev) {
  struct ice_adapter *ad = ICE_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  int i;
 ...
 * tx queue
 */
static inline int ice_check_fdir_programming_status(struct ice_rx_queue *rxq) {
...
int ice_fdir_programming(struct ice_pf *pf, struct ice_fltr_desc *fdir_desc) {
  ...
  fdirdp = (volatile struct ice_fltr_desc *)(&txq->tx_ring[txq->tx_tail]);
  ...
  td_cmd = ICE_TX_DESC_CMD_EOP | ICE_TX_DESC_CMD_RS | ICE_TX_DESC_CMD_DUMMY;

  txdp->cmd_type_offset_bsz = ice_build_ctob(td_cmd, 0, ICE_FDIR_PKT_LEN, 0);
  ...
  for (i = 0; i < ICE_FDIR_MAX_WAIT_US; i++) {
    if ((txdp->cmd_type_offset_bsz & rte_cpu_to_le_64(ICE_TXD_QW1_DTYPE_M)) ==
        rte_cpu_to_le_64(ICE_TX_DESC_DTYPE_DESC_DONE))
      break;
    rte_delay_us(1);
  }
  if (i >= ICE_FDIR_MAX_WAIT_US) {
    PMD_DRV_LOG(
        ERR, "Failed to program FDIR filter: time out to get DD on tx queue.");
    return -ETIMEDOUT;
  }
