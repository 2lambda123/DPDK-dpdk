...
static const char *const ice_valid_args[] = {
    ICE_SAFE_MODE_SUPPORT_ARG, ICE_PROTO_XTR_ARG,
    ICE_FIELD_OFFS_ARG,        ICE_FIELD_NAME_ARG,
    ICE_HW_DEBUG_MASK_ARG,     ICE_ONE_PPS_OUT_ARG,
    ICE_RX_LOW_LATENCY_ARG,    ICE_DEFAULT_MAC_DISABLE,
    ICE_MBUF_CHECK_ARG,        NULL};
...
static struct proto_xtr_ol_flag ice_proto_xtr_ol_flag_params[] = {
    [PROTO_XTR_VLAN] = {.param = {.name = "intel_pmd_dynflag_proto_xtr_vlan"}},
    [PROTO_XTR_IPV4] = {.param = {.name = "intel_pmd_dynflag_proto_xtr_ipv4"}},
    [PROTO_XTR_IPV6] = {.param = {.name = "intel_pmd_dynflag_proto_xtr_ipv6"}},
    [PROTO_XTR_IPV6_FLOW] =
        {.param = {.name = "intel_pmd_dynflag_proto_xtr_ipv6_flow"}},
    [PROTO_XTR_TCP] = {.param = {.name = "intel_pmd_dynflag_proto_xtr_tcp"}},
    [PROTO_XTR_IP_OFFSET] = {
...
static int ice_phy_conf_link(struct ice_hw *hw, u16 force_speed, bool link_up);
static int ice_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
static int ice_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                               int on);
static int ice_macaddr_set(struct rte_eth_dev *dev,
                           struct rte_ether_addr *mac_addr);
static int ice_macaddr_add(struct rte_eth_dev *dev,
                           struct rte_ether_addr *mac_addr,
                           __rte_unused uint32_t index, uint32_t pool);
static void ice_macaddr_remove(struct rte_eth_dev *dev, uint32_t index);
static int ice_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t queue_id);
...
static int ice_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                              size_t fw_size);
static int ice_vlan_pvid_set(struct rte_eth_dev *dev, uint16_t pvid, int on);
...
static int ice_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats);
static int ice_stats_reset(struct rte_eth_dev *dev);
static int ice_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                          unsigned int n);
...
ice_buffer_split_supported_hdr_ptypes_get(struct rte_eth_dev *dev,
...
static int ice_tm_ops_get(struct rte_eth_dev *dev __rte_unused, void *arg) {
  if (!arg)
    return -EINVAL;
...
static const struct ice_xstats_name_off ice_stats_strings[] = {
    ...
    {"rx_unknown_protocol_packets",
     offsetof(struct ice_eth_stats, rx_unknown_protocol)},
...
#define ICE_NB_ETH_XSTATS                                                      \
  (sizeof(ice_stats_strings) / sizeof(ice_stats_strings[0]))
...
static const struct ice_xstats_name_off ice_hw_port_strings[] = {
    {"tx_link_down_dropped",
     offsetof(struct ice_hw_port_stats, tx_dropped_link_down)},
    {"rx_crc_errors", offsetof(struct ice_hw_port_stats, crc_errors)},
    {"rx_illegal_byte_errors",
     offsetof(struct ice_hw_port_stats, illegal_bytes)},
    {"rx_error_bytes", offsetof(struct ice_hw_port_stats, error_bytes)},
    {"mac_local_errors", offsetof(struct ice_hw_port_stats, mac_local_faults)},
    {"mac_remote_errors",
     offsetof(struct ice_hw_port_stats, mac_remote_faults)},
    {"rx_len_errors", offsetof(struct ice_hw_port_stats, rx_len_errors)},
    ...
    {"rx_size_65_to_127_packets",
     offsetof(struct ice_hw_port_stats, rx_size_127)},
    {"rx_size_128_to_255_packets",
     offsetof(struct ice_hw_port_stats, rx_size_255)},
    {"rx_size_256_to_511_packets",
     offsetof(struct ice_hw_port_stats, rx_size_511)},
    {"rx_size_512_to_1023_packets",
     offsetof(struct ice_hw_port_stats, rx_size_1023)},
    {"rx_size_1024_to_1522_packets",
     offsetof(struct ice_hw_port_stats, rx_size_1522)},
    {"rx_size_1523_to_max_packets",
     offsetof(struct ice_hw_port_stats, rx_size_big)},
    {"rx_undersized_errors", offsetof(struct ice_hw_port_stats, rx_undersize)},
    {"rx_oversize_errors", offsetof(struct ice_hw_port_stats, rx_oversize)},
    {"rx_mac_short_pkt_dropped",
     offsetof(struct ice_hw_port_stats, mac_short_pkt_dropped)},
    {"rx_fragmented_errors", offsetof(struct ice_hw_port_stats, rx_fragments)},
    ...
    {"tx_size_65_to_127_packets",
     offsetof(struct ice_hw_port_stats, tx_size_127)},
    {"tx_size_128_to_255_packets",
     offsetof(struct ice_hw_port_stats, tx_size_255)},
    {"tx_size_256_to_511_packets",
     offsetof(struct ice_hw_port_stats, tx_size_511)},
    {"tx_size_512_to_1023_packets",
     offsetof(struct ice_hw_port_stats, tx_size_1023)},
    {"tx_size_1024_to_1522_packets",
     offsetof(struct ice_hw_port_stats, tx_size_1522)},
    {"tx_size_1523_to_max_packets",
     offsetof(struct ice_hw_port_stats, tx_size_big)},
};

#define ICE_NB_HW_PORT_XSTATS                                                  \
  (sizeof(ice_hw_port_strings) / sizeof(ice_hw_port_strings[0]))

static void ice_init_controlq_parameter(struct ice_hw *hw) {
  /* fields for adminq */
...
static int lookup_proto_xtr_type(const char *xtr_name) {
  static struct {
    const char *name;
    enum proto_xtr_type type;
  } xtr_type_map[] = {
      {"vlan", PROTO_XTR_VLAN}, {"ipv4", PROTO_XTR_IPV4},
      {"ipv6", PROTO_XTR_IPV6}, {"ipv6_flow", PROTO_XTR_IPV6_FLOW},
      {"tcp", PROTO_XTR_TCP},   {"ip_offset", PROTO_XTR_IP_OFFSET},
  };
  uint32_t i;
 ...
 */
static int parse_queue_set(const char *input, int xtr_type,
  ...
  if (*str != '(') {
    ...
    for (idx = RTE_MIN(min, max); idx <= RTE_MAX(min, max); idx++)
...
static int parse_queue_proto_xtr(const char *queues,
                                 struct ice_devargs *devargs) {
  ...
  do {
    ...
    for (idx = 0;; idx++) {
      if (isblank(queues[idx]) || queues[idx] == ',' || queues[idx] == ']' ||
          queues[idx] == '\0')
        break;
...
static int handle_proto_xtr_arg(__rte_unused const char *key, const char *value,
                                void *extra_args) {
  ...
  if (parse_queue_proto_xtr(value, devargs) < 0) {
    PMD_DRV_LOG(ERR, "The protocol extraction parameter is wrong : '%s'",
                value);
    return -1;
  }
...
static int handle_field_offs_arg(__rte_unused const char *key,
                                 const char *value, void *offs_args) {
...
static int handle_field_name_arg(__rte_unused const char *key,
                                 const char *value, void *name_args) {
  char *name = name_args;
  int ret;
  ...
  if (ret < 0 || ret >= RTE_MBUF_DYN_NAMESIZE) {
    PMD_DRV_LOG(ERR, "The protocol extraction field name too long : '%s'",
                name);
    return -1;
  }
  return 0;
...
static void ice_check_proto_xtr_support(struct ice_hw *hw) {
#define FLX_REG(val, fld, idx)                                                 \
  ...
  static struct {
    ...
    uint8_t protid_1;
      ...
      [PROTO_XTR_VLAN] = {ICE_RXDID_COMMS_AUX_VLAN, ICE_RX_OPC_EXTRACT,
                          ICE_PROT_EVLAN_O, ICE_PROT_VLAN_O},
      [PROTO_XTR_IPV4] = {ICE_RXDID_COMMS_AUX_IPV4, ICE_RX_OPC_EXTRACT,
                          ICE_PROT_IPV4_OF_OR_S, ICE_PROT_IPV4_OF_OR_S},
      [PROTO_XTR_IPV6] = {ICE_RXDID_COMMS_AUX_IPV6, ICE_RX_OPC_EXTRACT,
                          ICE_PROT_IPV6_OF_OR_S, ICE_PROT_IPV6_OF_OR_S},
      [PROTO_XTR_IPV6_FLOW] = {ICE_RXDID_COMMS_AUX_IPV6_FLOW,
                               ICE_RX_OPC_EXTRACT, ICE_PROT_IPV6_OF_OR_S,
                               ICE_PROT_IPV6_OF_OR_S},
      [PROTO_XTR_TCP] = {ICE_RXDID_COMMS_AUX_TCP, ICE_RX_OPC_EXTRACT,
                         ICE_PROT_TCP_IL, ICE_PROT_ID_INVAL},
      [PROTO_XTR_IP_OFFSET] = {ICE_RXDID_COMMS_AUX_IP_OFFSET, ICE_RX_OPC_PROTID,
                               ICE_PROT_IPV4_OF_OR_S, ICE_PROT_IPV6_OF_OR_S},
  };
  uint32_t i;
...
static int ice_res_pool_init(struct ice_res_pool_info *pool, uint32_t base,
                             uint32_t num) {
  ...
  if (!entry) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory for resource pool");
    return -ENOMEM;
  }
...
static int ice_res_pool_alloc(struct ice_res_pool_info *pool, uint16_t num) {
  ...
  if (pool->num_free < num) {
    PMD_INIT_LOG(ERR, "No resource. ask:%u, available:%u", num, pool->num_free);
    return -ENOMEM;
  }
  ...
  LIST_FOREACH(entry, &pool->free_list, next) {
    if (entry->len >= num) {
      ...
      if (!valid_entry || valid_entry->len > entry->len)
        valid_entry = entry;
    }
  }
...
static void ice_res_pool_destroy(struct ice_res_pool_info *pool) {
  ...
  for (entry = LIST_FIRST(&pool->alloc_list);
       entry && (next_entry = LIST_NEXT(entry, next), 1); entry = next_entry) {
...
static void ice_vsi_config_default_rss(struct ice_aqc_vsi_props *info) {
  /* Set VSI LUT selection */
  info->q_opt_rss = ICE_AQ_VSI_Q_OPT_RSS_LUT_VSI & ICE_AQ_VSI_Q_OPT_RSS_LUT_M;
  /* Set Hash scheme */
  info->q_opt_rss |= ICE_AQ_VSI_Q_OPT_RSS_TPLZ & ICE_AQ_VSI_Q_OPT_RSS_HASH_M;
  /* enable TC */
...
ice_vsi_config_tc_queue_mapping(struct ice_vsi *vsi,
  ...
  info->tc_mapping[0] = rte_cpu_to_le_16((qp_idx << ICE_AQ_VSI_TC_Q_OFFSET_S) |
  ...
  info->valid_sections |= rte_cpu_to_le_16(ICE_AQ_VSI_PROP_RXQ_MAP_VALID);
...
static int ice_init_mac_address(struct rte_eth_dev *dev) {
...
static int ice_add_mac_filter(struct ice_vsi *vsi,
                              struct rte_ether_addr *mac_addr) {
  ...
  if (ad->devargs.default_mac_disable == 1 &&
      rte_is_same_ether_addr(
          mac_addr, (struct rte_ether_addr *)hw->port_info[0].mac.perm_addr)) {
  ...
  m_list_itr =
      (struct ice_fltr_list_entry *)ice_malloc(hw, sizeof(*m_list_itr));
  if (!m_list_itr) {
    ret = -ENOMEM;
    goto DONE;
  }
  ice_memcpy(m_list_itr->fltr_info.l_data.mac.mac_addr, mac_addr, ETH_ALEN,
...
static int ice_remove_mac_filter(struct ice_vsi *vsi,
                                 struct rte_ether_addr *mac_addr) {
  struct ice_fltr_list_entry *m_list_itr = NULL;
  struct ice_mac_filter *f;
  struct LIST_HEAD_TYPE list_head;
...
static struct ice_vlan_filter *ice_find_vlan_filter(struct ice_vsi *vsi,
...
static int ice_add_vlan_filter(struct ice_vsi *vsi, struct ice_vlan *vlan) {
  ...
  v_list_itr =
      (struct ice_fltr_list_entry *)ice_malloc(hw, sizeof(*v_list_itr));
  if (!v_list_itr) {
    ret = -ENOMEM;
    goto DONE;
  }
...
static int ice_remove_vlan_filter(struct ice_vsi *vsi, struct ice_vlan *vlan) {
  struct ice_fltr_list_entry *v_list_itr = NULL;
  struct ice_vlan_filter *f;
  struct LIST_HEAD_TYPE list_head;
  struct ice_hw *hw;
...
static int ice_remove_all_mac_vlan_filters(struct ice_vsi *vsi) {
...
/* Enable IRQ0 */
static void ice_pf_enable_irq0(struct ice_hw *hw) {
  /* reset the registers */
  ...
  ICE_WRITE_REG(
      hw, PFINT_OICR_ENA,
      (uint32_t)(PFINT_OICR_ENA_INT_ENA_M & (~PFINT_OICR_LINK_STAT_CHANGE_M)));

  ICE_WRITE_REG(
      hw, PFINT_OICR_CTL,
      (0 & PFINT_OICR_CTL_MSIX_INDX_M) |
          ((0 << PFINT_OICR_CTL_ITR_INDX_S) & PFINT_OICR_CTL_ITR_INDX_M) |
          PFINT_OICR_CTL_CAUSE_ENA_M);

  ICE_WRITE_REG(hw, PFINT_FW_CTL,
                (0 & PFINT_FW_CTL_MSIX_INDX_M) |
                    ((0 << PFINT_FW_CTL_ITR_INDX_S) & PFINT_FW_CTL_ITR_INDX_M) |
...
/* Disable IRQ0 */
static void ice_pf_disable_irq0(struct ice_hw *hw) {
...
#ifdef ICE_LSE_SPT
static void ice_handle_aq_msg(struct rte_eth_dev *dev) {
  struct ice_hw *hw = ICE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  while (pending) {
    ...
    case ice_aqc_opc_get_link_status:
      ...
      break;
    default:
      PMD_DRV_LOG(DEBUG, "Request %u is not supported yet", opcode);
      break;
    }
  }
 ...
 *  void
 */
static void ice_interrupt_handler(void *param) {
  ...
  if (oicr & PFINT_OICR_MAL_DETECT_M) {
    ...
    if (reg & GL_MDET_TX_PQM_VALID_M) {
      pf_num = (reg & GL_MDET_TX_PQM_PF_NUM_M) >> GL_MDET_TX_PQM_PF_NUM_S;
      event = (reg & GL_MDET_TX_PQM_MAL_TYPE_M) >> GL_MDET_TX_PQM_MAL_TYPE_S;
      queue = (reg & GL_MDET_TX_PQM_QNUM_M) >> GL_MDET_TX_PQM_QNUM_S;
    ...
    if (reg & GL_MDET_TX_TCLAN_VALID_M) {
      pf_num = (reg & GL_MDET_TX_TCLAN_PF_NUM_M) >> GL_MDET_TX_TCLAN_PF_NUM_S;
      event =
          (reg & GL_MDET_TX_TCLAN_MAL_TYPE_M) >> GL_MDET_TX_TCLAN_MAL_TYPE_S;
      queue = (reg & GL_MDET_TX_TCLAN_QNUM_M) >> GL_MDET_TX_TCLAN_QNUM_S;
...
static void ice_init_proto_xtr(struct rte_eth_dev *dev) {
  ...
  if (ad->devargs.xtr_field_offs == field_offs) {
    PMD_DRV_LOG(DEBUG, "Protocol extraction metadata offset in mbuf is : %d",
                ad->devargs.xtr_field_offs);
  } else {
    PMD_DRV_LOG(ERR,
                "Invalid field offset or name, no match dynfield, [%d],[%s]",
  ...
  for (i = 0; i < RTE_DIM(ice_proto_xtr_ol_flag_params); i++) {
    ...
    if (!ice_proto_xtr_hw_support[i]) {
      PMD_DRV_LOG(
          ERR, "Protocol extraction type %u is not supported in hardware", i);
      ad->devargs.xtr_field_offs = -1;
      break;
    }
...
static int ice_pf_sw_init(struct rte_eth_dev *dev) {
  struct ice_pf *pf = ICE_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  struct ice_hw *hw = ICE_PF_TO_HW(pf);

  pf->lan_nb_qp_max = (uint16_t)RTE_MIN(hw->func_caps.common_cap.num_txq,
  ...
  if (hw->func_caps.fd_fltr_guar > 0 || hw->func_caps.fd_fltr_best_effort > 0) {
...
struct ice_vsi *ice_setup_vsi(struct ice_pf *pf, enum ice_vsi_type type) {
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  ...
  pf->hash_lut_size =
      hw->func_caps.common_cap.rss_table_size > RTE_ETH_RSS_RETA_SIZE_512
  ...
  case ICE_VSI_PF:
    ...
    vsi_ctx.info.q_opt_rss =
        ICE_AQ_VSI_Q_OPT_RSS_LUT_PF | ICE_AQ_VSI_Q_OPT_RSS_TPLZ;
    if (ice_is_dvm_ena(hw)) {
      vsi_ctx.info.outer_vlan_flags = (ICE_AQ_VSI_OUTER_VLAN_TX_MODE_ALL
                                       << ICE_AQ_VSI_OUTER_VLAN_TX_MODE_S) &
                                      ICE_AQ_VSI_OUTER_VLAN_TX_MODE_M;
      vsi_ctx.info.outer_vlan_flags |=
          (ICE_AQ_VSI_OUTER_TAG_VLAN_8100 << ICE_AQ_VSI_OUTER_TAG_TYPE_S) &
          ICE_AQ_VSI_OUTER_TAG_TYPE_M;
      vsi_ctx.info.outer_vlan_flags |= (ICE_AQ_VSI_OUTER_VLAN_EMODE_NOTHING
                                        << ICE_AQ_VSI_OUTER_VLAN_EMODE_S);
    }

    /* FDIR */
    cfg = ICE_AQ_VSI_PROP_SECURITY_VALID | ICE_AQ_VSI_PROP_FLOW_DIR_VALID;
    ...
    /* Enable VLAN/UP trip */
    ret =
        ice_vsi_config_tc_queue_mapping(vsi, &vsi_ctx.info, ICE_DEFAULT_TCMAP);
    if (ret) {
      PMD_INIT_LOG(ERR,
  ...
  if (type == ICE_VSI_PF) {
    ret = ice_res_pool_alloc(&pf->msix_pool,
                             RTE_MIN(vsi->nb_qps, RTE_MAX_RXTX_INTR_VEC_ID));
    if (ret < 0) {
      PMD_INIT_LOG(ERR, "VSI MAIN %d get heap failed %d", vsi->vsi_id, ret);
    }
    vsi->msix_intr = ret;
    ...
    if (ret < 0) {
      PMD_DRV_LOG(ERR, "VSI %d get heap failed %d", vsi->vsi_id, ret);
    }
    vsi->msix_intr = ret;
    vsi->nb_msix = 1;
  } else {
  ...
  if (type == ICE_VSI_PF) {
    /* MAC configuration */
    rte_ether_addr_copy((struct rte_ether_addr *)hw->port_info->mac.perm_addr,
                        &pf->dev_addr);
  ...
  ret = ice_cfg_vsi_lan(hw->port_info, vsi->idx, tc_bitmap, max_txqs);
  if (ret != ICE_SUCCESS)
...
static int ice_send_driver_ver(struct ice_hw *hw) {
...
static int ice_pf_setup(struct ice_pf *pf) {
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  struct ice_vsi *vsi;
  uint16_t unused;
  ...
  ice_alloc_fd_guar_item(hw, &unused, hw->func_caps.fd_fltr_guar);
  /* force shared filter pool for PF */
  ice_alloc_fd_shrd_item(hw, &unused, hw->func_caps.fd_fltr_best_effort);
...
static enum ice_pkg_type ice_load_pkg_type(struct ice_hw *hw) {
  ...
  PMD_INIT_LOG(NOTICE, "Active package is: %d.%d.%d.%d, %s (%s VLAN mode)",
               ...
               hw->active_pkg_name, ice_is_dvm_ena(hw) ? "double" : "single");
...
static void ice_base_queue_get(struct ice_pf *pf) {
  uint32_t reg;
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
...
static int parse_bool(const char *key, const char *value, void *args) {
  int *i = (int *)args;
  char *end;
  int num;
...
static int parse_u64(const char *key, const char *value, void *args) {
  u64 *num = (u64 *)args;
  u64 tmp;
  ...
  if (errno) {
    PMD_DRV_LOG(WARNING, "%s: \"%s\" is not a valid u64", key, value);
    return -1;
  }
...
static int lookup_pps_type(const char *pps_name) {
  static struct {
    const char *name;
    enum pps_type type;
  } pps_type_map[] = {
...
static int parse_pin_set(const char *input, int pps_type,
                         struct ice_devargs *devargs) {
  const char *str = input;
  char *end = NULL;
  uint32_t idx;
...
static int parse_pps_out_parameter(const char *pins,
                                   struct ice_devargs *devargs) {
  ...
  for (idx = 0;; idx++) {
    if (isblank(pins[idx]) || pins[idx] == ':' || pins[idx] == '\0')
      break;
...
static int handle_pps_out_arg(__rte_unused const char *key, const char *value,
                              void *extra_args) {
  struct ice_devargs *devargs = extra_args;
  ...
  if (parse_pps_out_parameter(value, devargs) < 0) {
    PMD_DRV_LOG(ERR, "The GPIO pin parameter is wrong : '%s'", value);
    return -1;
  }
...
static int ice_parse_mbuf_check(__rte_unused const char *key, const char *value,
                                void *args) {
  char *cur;
  char *tmp;
  int str_len;
...
static int ice_parse_devargs(struct rte_eth_dev *dev) {
  ...
  memset(ad->devargs.proto_xtr, PROTO_XTR_NONE, sizeof(ad->devargs.proto_xtr));

  ret = rte_kvargs_process(kvlist, ICE_PROTO_XTR_ARG, &handle_proto_xtr_arg,
                           &ad->devargs);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_FIELD_OFFS_ARG, &handle_field_offs_arg,
                           &ad->devargs.xtr_field_offs);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_FIELD_NAME_ARG, &handle_field_name_arg,
                           &ad->devargs.xtr_field_name);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_SAFE_MODE_SUPPORT_ARG, &parse_bool,
                           &ad->devargs.safe_mode_support);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_DEFAULT_MAC_DISABLE, &parse_bool,
                           &ad->devargs.default_mac_disable);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_HW_DEBUG_MASK_ARG, &parse_u64,
                           &ad->hw.debug_mask);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_ONE_PPS_OUT_ARG, &handle_pps_out_arg,
                           &ad->devargs);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_MBUF_CHECK_ARG, &ice_parse_mbuf_check,
                           &ad->devargs.mbuf_check);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, ICE_RX_LOW_LATENCY_ARG, &parse_bool,
...
static int ice_vsi_config_sw_lldp(struct ice_vsi *vsi, bool on) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  ...
  s_list_itr =
      (struct ice_fltr_list_entry *)ice_malloc(hw, sizeof(*s_list_itr));
  if (!s_list_itr)
    return -ENOMEM;
  s_list_itr->fltr_info.lkup_type = ICE_SW_LKUP_ETHERTYPE;
  s_list_itr->fltr_info.vsi_handle = vsi->idx;
  s_list_itr->fltr_info.l_data.ethertype_mac.ethertype = RTE_ETHER_TYPE_LLDP;
...
static enum ice_status ice_get_hw_res(struct ice_hw *hw, uint16_t res_type,
  ...
  struct ice_aqc_get_allocd_res_desc *cmd = &aq_desc.params.get_res_desc;
  ...
  ice_fill_dflt_direct_cmd_desc(&aq_desc, ice_aqc_opc_get_allocd_res_desc);

  cmd->ops.cmd.res =
      CPU_TO_LE16(((res_type << ICE_AQC_RES_TYPE_S) & ICE_AQC_RES_TYPE_M) |
                  (res_shared ? ICE_AQC_RES_TYPE_FLAG_SHARED : 0));
  ...
  ice_memcpy(prof_buf, resp_buf, sizeof(*resp_buf) * (*num_prof),
             ICE_NONDMA_TO_NONDMA);
...
static int ice_cleanup_resource(struct ice_hw *hw, uint16_t res_type) {
  int ret;
  uint16_t prof_id;
  ...
  ret = ice_get_hw_res(hw, res_type, ICE_MAX_RES_DESC_NUM, first_desc, prof_buf,
                       &num_prof);
  if (ret) {
...
static int ice_reset_fxp_resource(struct ice_hw *hw) {
  int ret;
...
static void ice_rss_ctx_init(struct ice_pf *pf) {
  memset(&pf->hash_ctx, 0, sizeof(pf->hash_ctx));
}

static uint64_t ice_get_supported_rxdid(struct ice_hw *hw) {
...
static int ice_dev_init(struct rte_eth_dev *dev) {
  ...
  PMD_INIT_LOG(INFO, "FW %d.%d.%05d API %d.%d", hw->fw_maj_ver, hw->fw_min_ver,
               hw->fw_build, hw->api_maj_ver, hw->api_min_ver);
  ...
  rte_intr_callback_register(intr_handle, ice_interrupt_handler, dev);
...
err_flow_init:
  ...
  ice_pf_disable_irq0(hw);
  rte_intr_callback_unregister(intr_handle, ice_interrupt_handler, dev);
err_pf_setup:
...
int ice_release_vsi(struct ice_vsi *vsi) {
  struct ice_hw *hw;
  struct ice_vsi_ctx vsi_ctx;
  enum ice_status ret;
  int error = 0;
...
static int ice_dev_close(struct rte_eth_dev *dev) {
   ...
   */
  rte_intr_callback_unregister_sync(intr_handle, ice_interrupt_handler, dev);
...
static int ice_dev_uninit(struct rte_eth_dev *dev) {
  ice_dev_close(dev);
...
static int ice_hash_moveout(struct ice_pf *pf, struct ice_rss_hash_cfg *cfg) {
  ...
  if (status && status != ICE_ERR_DOES_NOT_EXIST) {
    PMD_DRV_LOG(ERR, "ice_rem_rss_cfg failed for VSI:%d, error:%d\n", vsi->idx,
                status);
    return -EBUSY;
  }
...
static int ice_hash_moveback(struct ice_pf *pf, struct ice_rss_hash_cfg *cfg) {
  enum ice_status status = ICE_SUCCESS;
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  struct ice_vsi *vsi = pf->main_vsi;
  ...
  if (status) {
    PMD_DRV_LOG(ERR, "ice_add_rss_cfg failed for VSI:%d, error:%d\n", vsi->idx,
                status);
    return -EBUSY;
  }
...
static int ice_hash_remove(struct ice_pf *pf, struct ice_rss_hash_cfg *cfg) {
  int ret;
...
static int ice_add_rss_cfg_pre_gtpu(struct ice_pf *pf,
                                    struct ice_hash_gtpu_ctx *ctx, u8 ctx_idx) {
  int ret;

  switch (ctx_idx) {
  case ICE_HASH_GTPU_CTX_EH_IP:
    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP]);
    if (ret && (ret != -ENOENT))
      return ret;
    ...
    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_remove(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;
  ...
  case ICE_HASH_GTPU_CTX_EH_IP_UDP:
    ...
    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;
  ...
  case ICE_HASH_GTPU_CTX_EH_IP_TCP:
    ...
    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;
    ...
    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;
  ...
  case ICE_HASH_GTPU_CTX_UP_IP:
    ...
    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveout(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;
...
static int ice_add_rss_cfg_pre(struct ice_pf *pf, uint32_t hdr) {
  u8 gtpu_ctx_idx = calc_gtpu_ctx_idx(hdr);

  if (hdr & ICE_FLOW_SEG_HDR_IPV4)
    return ice_add_rss_cfg_pre_gtpu(pf, &pf->hash_ctx.gtpu4, gtpu_ctx_idx);
  else if (hdr & ICE_FLOW_SEG_HDR_IPV6)
    return ice_add_rss_cfg_pre_gtpu(pf, &pf->hash_ctx.gtpu6, gtpu_ctx_idx);

  return 0;
}

static int ice_add_rss_cfg_post_gtpu(struct ice_pf *pf,
                                     struct ice_hash_gtpu_ctx *ctx, u8 ctx_idx,
  ...
  case ICE_HASH_GTPU_CTX_EH_IP_UDP:
    ...
    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;
  ...
  case ICE_HASH_GTPU_CTX_EH_IP_TCP:
    ...
    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_UP_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;
    ...
    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_DW_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;
  ...
  case ICE_HASH_GTPU_CTX_DW_IP_UDP:
  case ICE_HASH_GTPU_CTX_DW_IP_TCP:
    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_UDP]);
    if (ret && (ret != -ENOENT))
      return ret;

    ret = ice_hash_moveback(pf, &ctx->ctx[ICE_HASH_GTPU_CTX_EH_IP_TCP]);
    if (ret && (ret != -ENOENT))
      return ret;
...
static int ice_add_rss_cfg_post(struct ice_pf *pf,
                                struct ice_rss_hash_cfg *cfg) {
  u8 gtpu_ctx_idx = calc_gtpu_ctx_idx(cfg->addl_hdrs);

  if (cfg->addl_hdrs & ICE_FLOW_SEG_HDR_IPV4)
    return ice_add_rss_cfg_post_gtpu(pf, &pf->hash_ctx.gtpu4, gtpu_ctx_idx,
                                     cfg);
  else if (cfg->addl_hdrs & ICE_FLOW_SEG_HDR_IPV6)
    return ice_add_rss_cfg_post_gtpu(pf, &pf->hash_ctx.gtpu6, gtpu_ctx_idx,
                                     cfg);

  return 0;
}

static void ice_rem_rss_cfg_post(struct ice_pf *pf, uint32_t hdr) {
  u8 gtpu_ctx_idx = calc_gtpu_ctx_idx(hdr);
...
static void ice_rss_hash_set(struct ice_pf *pf, uint64_t rss_hf) {
  struct ice_hw *hw = ICE_PF_TO_HW(pf);
  struct ice_vsi *vsi = pf->main_vsi;
  struct ice_rss_hash_cfg cfg;
  int ret;
...
#define ICE_RSS_HF_ALL                                                         \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV4_UDP |        \
   RTE_ETH_RSS_NONFRAG_IPV6_UDP | RTE_ETH_RSS_NONFRAG_IPV4_TCP |               \
   RTE_ETH_RSS_NONFRAG_IPV6_TCP | RTE_ETH_RSS_NONFRAG_IPV4_SCTP |              \
   RTE_ETH_RSS_NONFRAG_IPV6_SCTP)

  ret = ice_rem_vsi_rss_cfg(hw, vsi->idx);
  if (ret)
    PMD_DRV_LOG(ERR, "%s Remove rss vsi fail %d", __func__, ret);
  ...
  if (rss_hf & RTE_ETH_RSS_IPV4) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV4 rss flow fail %d", __func__, ret);
  }
  ...
  if (rss_hf & RTE_ETH_RSS_IPV6) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV6 rss flow fail %d", __func__, ret);
  }
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV4_UDP) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV4_UDP rss flow fail %d", __func__, ret);
  }
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV6_UDP) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV6_UDP rss flow fail %d", __func__, ret);
  }
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV4_TCP) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV4_TCP rss flow fail %d", __func__, ret);
  }
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV6_TCP) {
    ...
    ret = ice_add_rss_cfg_wrap(pf, vsi->idx, &cfg);
    if (ret)
      PMD_DRV_LOG(ERR, "%s PPPoE_IPV6_TCP rss flow fail %d", __func__, ret);
  }

  pf->rss_hf = rss_hf & ICE_RSS_HF_ALL;
}

static void ice_get_default_rss_key(uint8_t *rss_key, uint32_t rss_key_size) {
  ...
  if (rss_key_size > sizeof(default_key)) {
    PMD_DRV_LOG(WARNING,
                ...
                rss_key_size, sizeof(default_key), sizeof(default_key));
  }
...
static int ice_init_rss(struct ice_pf *pf) {
  ...
  if (nb_q == 0) {
    PMD_DRV_LOG(WARNING, "RSS is not supported as rx queues number is zero\n");
    return 0;
  }
  ...
  if (!vsi->rss_key) {
    vsi->rss_key = rte_zmalloc(NULL, vsi->rss_key_size, 0);
    if (vsi->rss_key == NULL) {
  ...
  if (!vsi->rss_lut) {
    vsi->rss_lut = rte_zmalloc(NULL, vsi->rss_lut_size, 0);
    if (vsi->rss_lut == NULL) {
      PMD_DRV_LOG(ERR, "Failed to allocate memory for rss_key");
      rte_free(vsi->rss_key);
      vsi->rss_key = NULL;
...
static void __vsi_queues_bind_intr(struct ice_vsi *vsi, uint16_t msix_vect,
  ...
  for (i = 0; i < nb_queue; i++) {
    /*do actual bind*/
    val = (msix_vect & QINT_RQCTL_MSIX_INDX_M) | (0 << QINT_RQCTL_ITR_INDX_S) |
          QINT_RQCTL_CAUSE_ENA_M;
    val_tx = (msix_vect & QINT_TQCTL_MSIX_INDX_M) |
             (0 << QINT_TQCTL_ITR_INDX_S) | QINT_TQCTL_CAUSE_ENA_M;

    PMD_DRV_LOG(INFO, "queue %d is binding to vect %d", base_queue + i,
                msix_vect);
...
void ice_vsi_queues_bind_intr(struct ice_vsi *vsi) {
  ...
  uint16_t nb_msix = RTE_MIN(vsi->nb_msix, rte_intr_nb_efd_get(intr_handle));
  ...
  for (i = 0; i < vsi->nb_used_qps; i++) {
    if (nb_msix <= 1) {
      ...
      __vsi_queues_bind_intr(vsi, msix_vect, vsi->base_queue + i,
                             vsi->nb_used_qps - i);

      for (; !!record && i < vsi->nb_used_qps; i++)
        rte_intr_vec_list_index_set(intr_handle, queue_idx + i, msix_vect);

      break;
    }

    /* vfio 1:1 queue/msix_vect mapping */
    __vsi_queues_bind_intr(vsi, msix_vect, vsi->base_queue + i, 1);
  ...
  if (rte_intr_allow_others(intr_handle))
    for (i = 0; i < vsi->nb_used_qps; i++) {
      ...
      ICE_WRITE_REG(hw, GLINT_DYN_CTL(msix_intr),
                    GLINT_DYN_CTL_INTENA_M | GLINT_DYN_CTL_CLEARPBA_M |
                        GLINT_DYN_CTL_ITR_INDX_M | GLINT_DYN_CTL_WB_ON_ITR_M);
    }
  else
    ICE_WRITE_REG(hw, GLINT_DYN_CTL(0),
                  GLINT_DYN_CTL_INTENA_M | GLINT_DYN_CTL_CLEARPBA_M |
...
static int ice_rxq_intr_setup(struct rte_eth_dev *dev) {
  struct ice_pf *pf = ICE_DEV_PRIVATE_TO_PF(dev->data->dev_private);
  struct rte_pci_device *pci_dev = ICE_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;
  struct ice_vsi *vsi = pf->main_vsi;
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    if (rte_intr_vec_list_alloc(intr_handle, NULL, dev->data->nb_rx_queues)) {
...
static enum ice_status ice_get_link_info_safe(struct ice_pf *pf, bool ena_lse,
...
static int ice_pps_out_cfg(struct ice_hw *hw, int idx, int timer) {
  ...
  start_time = (current_time + NSEC_PER_SEC) / NSEC_PER_SEC * NSEC_PER_SEC;
...
static int ice_dev_start(struct rte_eth_dev *dev) {
  ...
  ret = ice_aq_set_event_mask(
      hw, hw->port_info->lport,
      ((u16)(ICE_AQ_LINK_EVENT_LINK_FAULT | ICE_AQ_LINK_EVENT_PHY_TEMP_ALARM |
  ...
  max_frame_size = pf->dev_data->mtu ? pf->dev_data->mtu + ICE_ETH_OVERHEAD
                                     : ICE_FRAME_SIZE_MAX;
...
static int ice_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      ...
      RTE_ETH_RX_OFFLOAD_SCATTER | RTE_ETH_RX_OFFLOAD_VLAN_FILTER;
  dev_info->tx_offload_capa =
      RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_TCP_TSO |
  ...
  if (!is_safe_mode) {
    dev_info->rx_offload_capa |=
        ...
        RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_QINQ_STRIP |
        RTE_ETH_RX_OFFLOAD_OUTER_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_VLAN_EXTEND |
    ...
    dev_info->tx_offload_capa |=
        ...
        RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO |
  ...
  dev_info->speed_capa = RTE_ETH_LINK_SPEED_10M | RTE_ETH_LINK_SPEED_100M |
                         ...
                         RTE_ETH_LINK_SPEED_20G | RTE_ETH_LINK_SPEED_25G;
...
static inline int ice_atomic_read_link_status(struct rte_eth_dev *dev,
  ...
  if (!__atomic_compare_exchange_n((uint64_t *)dst, (uint64_t *)dst,
                                   *(uint64_t *)src, 0, __ATOMIC_SEQ_CST,
                                   __ATOMIC_SEQ_CST))
    return -1;
...
static inline int ice_atomic_write_link_status(struct rte_eth_dev *dev,
                                               struct rte_eth_link *link) {
...
static inline uint16_t ice_parse_link_speeds(uint16_t link_speeds) {
...
static int ice_apply_link_speed(struct rte_eth_dev *dev) {
  uint16_t speed;
  struct ice_hw *hw = ICE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_eth_conf *conf = &dev->data->dev_conf;

  if (conf->link_speeds == RTE_ETH_LINK_SPEED_AUTONEG) {
    conf->link_speeds = RTE_ETH_LINK_SPEED_100G | RTE_ETH_LINK_SPEED_50G |
                        RTE_ETH_LINK_SPEED_40G | RTE_ETH_LINK_SPEED_25G |
                        RTE_ETH_LINK_SPEED_20G | RTE_ETH_LINK_SPEED_10G |
                        RTE_ETH_LINK_SPEED_5G | RTE_ETH_LINK_SPEED_2_5G |
                        RTE_ETH_LINK_SPEED_1G | RTE_ETH_LINK_SPEED_100M;
  }
...
static int ice_phy_conf_link(struct ice_hw *hw, u16 link_speeds_bitmap,
                             bool link_up) {
  ...
  phy_caps =
      (struct ice_aqc_get_phy_caps_data *)ice_malloc(hw, sizeof(*phy_caps));
  if (!phy_caps)
  ...
  if (ice_fw_supports_report_dflt_cfg(pi->hw))
    err =
        ice_aq_get_phy_caps(pi, false, ICE_AQC_REPORT_DFLT_CFG, phy_caps, NULL);
  else
...
static int ice_mtu_set(struct rte_eth_dev *dev, uint16_t mtu __rte_unused) {
 ...
 * part of filtering.
 */
static int ice_vsi_add_vlan_zero(struct ice_vsi *vsi) {
  struct ice_vlan vlan;
  int err;
 ...
 */
static int ice_vsi_del_vlan_zero(struct ice_vsi *vsi) {
  struct ice_vlan vlan;
  int err;
...
static int ice_vsi_config_vlan_filter(struct ice_vsi *vsi, bool on) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  ...
  ctxt.info.valid_sections = rte_cpu_to_le_16(ICE_AQ_VSI_PROP_SW_VALID |
  ...
  if (ret) {
    ...
    vsi->info.valid_sections |= rte_cpu_to_le_16(
        ICE_AQ_VSI_PROP_SW_VALID | ICE_AQ_VSI_PROP_SECURITY_VALID);
  }
...
static int ice_vsi_ena_inner_stripping(struct ice_vsi *vsi) {
  return ice_vsi_manage_vlan_stripping(vsi, true);
}

static int ice_vsi_dis_inner_stripping(struct ice_vsi *vsi) {
 ...
 * types: RTE_ETHER_TYPE_VLAN, RTE_ETHER_TYPE_QINQ1 and RTE_ETHER_TYPE_QINQ. If
 * the device is configured in Single VLAN Mode (SVM), then only
 * RTE_ETHER_TYPE_VLAN is supported.
 */
static bool ice_is_supported_port_vlan_proto(struct ice_hw *hw,
                                             u16 vlan_proto) {
...
static int ice_vsi_ena_outer_stripping(struct ice_vsi *vsi, u16 tpid) {
  ...
  ctxt.info.valid_sections = rte_cpu_to_le_16(ICE_AQ_VSI_PROP_OUTER_TAG_VALID);
  ...
  ctxt.info.outer_vlan_flags |=
      ...
      ((tag_type << ICE_AQ_VSI_OUTER_TAG_TYPE_S) & ICE_AQ_VSI_OUTER_TAG_TYPE_M);
...
static int ice_vsi_dis_outer_stripping(struct ice_vsi *vsi) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  struct ice_vsi_ctx ctxt;
  enum ice_status status;
  int err = 0;
...
static int ice_vsi_config_vlan_stripping(struct ice_vsi *vsi, bool ena) {
  int ret;
...
static void ice_vsi_update_l2tsel(struct ice_vsi *vsi, enum ice_l2tsel l2tsel) {
  ...
  for (i = 0; i < dev_data->nb_rx_queues; i++) {
    ...
    qrx_context_offset = QRX_CONTEXT(ICE_L2TSEL_QRX_CONTEXT_REG_IDX, i);
...
static int ice_vsi_config_outer_vlan_stripping(struct ice_vsi *vsi, bool on) {
  ...
  if (on) {
    ...
    if (!err) {
      enum ice_l2tsel l2tsel = ICE_L2TSEL_EXTRACT_FIRST_TAG_L2TAG2_2ND;
    ...
    if (!err) {
      enum ice_l2tsel l2tsel = ICE_L2TSEL_EXTRACT_FIRST_TAG_L2TAG1;
...
static int ice_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
  ...
  if (!ice_is_dvm_ena(hw)) {
    ...
    if ((mask & RTE_ETH_VLAN_STRIP_MASK) | (mask & RTE_ETH_QINQ_STRIP_MASK)) {
      if (rxmode->offloads &
...
static int ice_get_rss_lut(struct ice_vsi *vsi, uint8_t *lut,
                           uint16_t lut_size) {
  struct ice_aq_get_set_rss_lut_params lut_params;
  struct ice_pf *pf = ICE_VSI_TO_PF(vsi);
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
...
static int ice_set_rss_lut(struct ice_vsi *vsi, uint8_t *lut,
                           uint16_t lut_size) {
  struct ice_aq_get_set_rss_lut_params lut_params;
  struct ice_pf *pf;
  struct ice_hw *hw;
...
static int ice_set_rss_key(struct ice_vsi *vsi, uint8_t *key, uint8_t key_len) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  int ret = 0;
...
static int ice_get_rss_key(struct ice_vsi *vsi, uint8_t *key,
                           uint8_t *key_len) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  int ret;
  ...
  ret =
      ice_aq_get_rss_key(hw, vsi->idx, (struct ice_aqc_get_set_rss_keys *)key);
  if (ret) {
...
static int ice_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  ice_get_rss_key(vsi, rss_conf->rss_key, &rss_conf->rss_key_len);
...
static int ice_promisc_enable(struct rte_eth_dev *dev) {
  ...
  pmask = ICE_PROMISC_UCAST_RX | ICE_PROMISC_UCAST_TX | ICE_PROMISC_MCAST_RX |
...
static int ice_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
  ...
  ret = snprintf(fw_version, fw_size, "%x.%02x 0x%08x %d.%d.%d",
                 hw->flash.nvm.major, hw->flash.nvm.minor,
                 hw->flash.nvm.eetrack, ver, build, patch);
  if (ret < 0)
    return -EINVAL;
...
static int ice_vsi_vlan_pvid_set(struct ice_vsi *vsi,
  ...
  vsi->info.inner_vlan_flags &=
      ~(ICE_AQ_VSI_INNER_VLAN_INSERT_PVID | ICE_AQ_VSI_INNER_VLAN_EMODE_M);
  ...
  ret = ice_update_vsi(hw, vsi->idx, &ctxt, NULL);
  if (ret != ICE_SUCCESS) {
    PMD_DRV_LOG(ERR, "update VSI for VLAN insert failed, err %d", ret);
    return -EINVAL;
  }

  vsi->info.valid_sections |= rte_cpu_to_le_16(ICE_AQ_VSI_PROP_VLAN_VALID);
 ...
 */
static int ice_vsi_set_outer_port_vlan(struct ice_vsi *vsi, u16 vlan_info,
                                       u16 tpid) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  struct ice_vsi_ctx ctxt;
  enum ice_status status;
 ...
 */
static int ice_vsi_dis_outer_insertion(struct ice_vsi *vsi,
                                       struct ice_vsi_vlan_pvid_info *info) {
  struct ice_hw *hw = ICE_VSI_TO_HW(vsi);
  struct ice_vsi_ctx ctxt;
  enum ice_status status;
  ...
  ctxt.info.outer_vlan_flags |=
      ICE_AQ_VSI_OUTER_VLAN_BLOCK_TX_DESC |
      ((vlan_flags << ICE_AQ_VSI_OUTER_VLAN_TX_MODE_S) &
       ICE_AQ_VSI_OUTER_VLAN_TX_MODE_M);
...
static int ice_vlan_pvid_set(struct rte_eth_dev *dev, uint16_t pvid, int on) {
  ...
  if (info.on) {
    ...
    info.config.reject.tagged = data->dev_conf.txmode.hw_vlan_reject_tagged;
    info.config.reject.untagged = data->dev_conf.txmode.hw_vlan_reject_untagged;
  }
...
static int ice_vsi_ena_outer_insertion(struct ice_vsi *vsi, uint16_t tpid) {
  ...
  ctxt.info.outer_vlan_flags =
      vsi->info.outer_vlan_flags &
      ~(ICE_AQ_VSI_OUTER_VLAN_PORT_BASED_INSERT |
        ICE_AQ_VSI_OUTER_VLAN_BLOCK_TX_DESC | ICE_AQ_VSI_OUTER_VLAN_TX_MODE_M |
        ICE_AQ_VSI_OUTER_TAG_TYPE_M);
  ctxt.info.outer_vlan_flags |=
      ((ICE_AQ_VSI_OUTER_VLAN_TX_MODE_ALL << ICE_AQ_VSI_OUTER_VLAN_TX_MODE_S) &
       ICE_AQ_VSI_OUTER_VLAN_TX_MODE_M) |
      ((tag_type << ICE_AQ_VSI_OUTER_TAG_TYPE_S) & ICE_AQ_VSI_OUTER_TAG_TYPE_M);
...
static int ice_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  struct ice_vsi *vsi = pf->main_vsi;
  uint64_t qinq =
      dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_QINQ_STRIP;
  int err = 0;
...
static int ice_get_eeprom(struct rte_eth_dev *dev,
  ...
  status = ice_read_flat_nvm(hw, eeprom->offset, &eeprom->length, data, false);
...
static int ice_get_module_info(struct rte_eth_dev *dev,
  ...
  status = ice_aq_sff_eeprom(hw, 0, ICE_I2C_EEPROM_DEV_ADDR, 0x00, 0x00, 0,
                             &value, 1, 0, NULL);
  if (status)
    return -EIO;
  ...
  case ICE_MODULE_TYPE_SFP:
    status = ice_aq_sff_eeprom(hw, 0, ICE_I2C_EEPROM_DEV_ADDR,
                               ICE_MODULE_SFF_8472_COMP, 0x00, 0, &sff8472_comp,
                               1, 0, NULL);
    if (status)
      return -EIO;
    status = ice_aq_sff_eeprom(hw, 0, ICE_I2C_EEPROM_DEV_ADDR,
                               ICE_MODULE_SFF_8472_SWAP, 0x00, 0, &sff8472_swap,
                               1, 0, NULL);
    if (status)
      return -EIO;
    ...
    } else if (sff8472_comp && (sff8472_swap & ICE_MODULE_SFF_DIAG_CAPAB)) {
  ...
  case ICE_MODULE_TYPE_QSFP28:
    status = ice_aq_sff_eeprom(hw, 0, ICE_I2C_EEPROM_DEV_ADDR,
                               ICE_MODULE_REVISION_ADDR, 0x00, 0, &sff8636_rev,
                               1, 0, NULL);
    if (status)
      return -EIO;
...
#define I2C_USLEEP_MAX_TIME 2500
  ...
  status = ice_aq_sff_eeprom(hw, 0, addr, offset, page, 0, value, 1, 0, NULL);
  if (status)
    return -EIO;
  ...
  for (i = 0; i < info->length; i += SFF_READ_BLOCK_SIZE) {
    ...
    if (page == 0 || !(data[0x2] & 0x4)) {
      ...
      for (j = 0; j < I2C_BUSY_TRY_TIMES; j++) {
        status = ice_aq_sff_eeprom(hw, 0, addr, offset, page, !is_sfp, value,
                                   SFF_READ_BLOCK_SIZE, 0, NULL);
        PMD_DRV_LOG(DEBUG,
                    "SFF %02X %02X %02X %X = "
                    "%02X%02X%02X%02X."
                    "%02X%02X%02X%02X (%X)\n",
                    addr, offset, page, is_sfp, value[0], value[1], value[2],
                    value[3], value[4], value[5], value[6], value[7], status);
        if (status) {
          usleep_range(I2C_USLEEP_MIN_TIME, I2C_USLEEP_MAX_TIME);
...
static void ice_stat_update_32(struct ice_hw *hw, uint32_t reg,
                               bool offset_loaded, uint64_t *offset,
                               uint64_t *stat) {
  uint64_t new_data;
  ...
  if (new_data >= *offset)
    ...
    *stat =
        (uint64_t)((new_data + ((uint64_t)1 << ICE_32_BIT_WIDTH)) - *offset);
}

static void ice_stat_update_40(struct ice_hw *hw, uint32_t hireg,
                               uint32_t loreg, bool offset_loaded,
                               uint64_t *offset, uint64_t *stat) {
  uint64_t new_data;
  ...
  if (new_data >= *offset)
    ...
    *stat =
        (uint64_t)((new_data + ((uint64_t)1 << ICE_40_BIT_WIDTH)) - *offset);
...
static void ice_update_vsi_stats(struct ice_vsi *vsi) {
  ...
  ice_stat_update_40(hw, GLV_GORCH(idx), GLV_GORCL(idx), vsi->offset_loaded,
                     &oes->rx_bytes, &nes->rx_bytes);
  ice_stat_update_40(hw, GLV_UPRCH(idx), GLV_UPRCL(idx), vsi->offset_loaded,
                     &oes->rx_unicast, &nes->rx_unicast);
  ice_stat_update_40(hw, GLV_MPRCH(idx), GLV_MPRCL(idx), vsi->offset_loaded,
                     &oes->rx_multicast, &nes->rx_multicast);
  ice_stat_update_40(hw, GLV_BPRCH(idx), GLV_BPRCL(idx), vsi->offset_loaded,
  ...
  ice_stat_update_32(hw, GLV_RDPC(idx), vsi->offset_loaded, &oes->rx_discards,
                     &nes->rx_discards);
  /* GLV_REPC not supported */
  /* GLV_RMPC not supported */
  ice_stat_update_32(hw, GLSWID_RUPP(idx), vsi->offset_loaded,
                     &oes->rx_unknown_protocol, &nes->rx_unknown_protocol);
  ice_stat_update_40(hw, GLV_GOTCH(idx), GLV_GOTCL(idx), vsi->offset_loaded,
                     &oes->tx_bytes, &nes->tx_bytes);
  ice_stat_update_40(hw, GLV_UPTCH(idx), GLV_UPTCL(idx), vsi->offset_loaded,
                     &oes->tx_unicast, &nes->tx_unicast);
  ice_stat_update_40(hw, GLV_MPTCH(idx), GLV_MPTCL(idx), vsi->offset_loaded,
                     &oes->tx_multicast, &nes->tx_multicast);
  ice_stat_update_40(hw, GLV_BPTCH(idx), GLV_BPTCL(idx), vsi->offset_loaded,
                     &oes->tx_broadcast, &nes->tx_broadcast);
  /* GLV_TDPC not supported */
  ice_stat_update_32(hw, GLV_TEPC(idx), vsi->offset_loaded, &oes->tx_errors,
                     &nes->tx_errors);
...
static void ice_read_stats_registers(struct ice_pf *pf, struct ice_hw *hw) {
  ...
  ice_stat_update_40(hw, GLPRT_GORCH(hw->port_info->lport),
                     GLPRT_GORCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.rx_bytes, &ns->eth.rx_bytes);
  ice_stat_update_40(hw, GLPRT_UPRCH(hw->port_info->lport),
                     GLPRT_UPRCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.rx_unicast, &ns->eth.rx_unicast);
  ice_stat_update_40(hw, GLPRT_MPRCH(hw->port_info->lport),
                     GLPRT_MPRCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.rx_multicast, &ns->eth.rx_multicast);
  ice_stat_update_40(hw, GLPRT_BPRCH(hw->port_info->lport),
                     GLPRT_BPRCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.rx_broadcast, &ns->eth.rx_broadcast);
  ice_stat_update_32(hw, PRTRPB_RDPC, pf->offset_loaded, &os->eth.rx_discards,
                     &ns->eth.rx_discards);
  /* enlarge the limitation when rx_bytes overflowed */
  if (pf->offset_loaded) {
   ...
   */
  ns->eth.rx_bytes -=
      (ns->eth.rx_unicast + ns->eth.rx_multicast + ns->eth.rx_broadcast) *
      RTE_ETHER_CRC_LEN;

  /* GLPRT_REPC not supported */
  /* GLPRT_RMPC not supported */
  ice_stat_update_32(hw, GLSWID_RUPP(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.rx_unknown_protocol,
                     &ns->eth.rx_unknown_protocol);
  ice_stat_update_40(hw, GLPRT_GOTCH(hw->port_info->lport),
                     GLPRT_GOTCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.tx_bytes, &ns->eth.tx_bytes);
  ice_stat_update_40(hw, GLPRT_UPTCH(hw->port_info->lport),
                     GLPRT_UPTCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.tx_unicast, &ns->eth.tx_unicast);
  ice_stat_update_40(hw, GLPRT_MPTCH(hw->port_info->lport),
                     GLPRT_MPTCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.tx_multicast, &ns->eth.tx_multicast);
  ice_stat_update_40(hw, GLPRT_BPTCH(hw->port_info->lport),
                     GLPRT_BPTCL(hw->port_info->lport), pf->offset_loaded,
                     &os->eth.tx_broadcast, &ns->eth.tx_broadcast);
  /* enlarge the limitation when tx_bytes overflowed */
  if (pf->offset_loaded) {
  ...
  ns->eth.tx_bytes -=
      (ns->eth.tx_unicast + ns->eth.tx_multicast + ns->eth.tx_broadcast) *
      RTE_ETHER_CRC_LEN;

  /* GLPRT_TEPC not supported */

  /* additional port specific stats */
  ice_stat_update_32(hw, GLPRT_TDOLD(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_dropped_link_down, &ns->tx_dropped_link_down);
  ice_stat_update_32(hw, GLPRT_CRCERRS(hw->port_info->lport), pf->offset_loaded,
                     &os->crc_errors, &ns->crc_errors);
  ice_stat_update_32(hw, GLPRT_ILLERRC(hw->port_info->lport), pf->offset_loaded,
                     &os->illegal_bytes, &ns->illegal_bytes);
  /* GLPRT_ERRBC not supported */
  ice_stat_update_32(hw, GLPRT_MLFC(hw->port_info->lport), pf->offset_loaded,
                     &os->mac_local_faults, &ns->mac_local_faults);
  ice_stat_update_32(hw, GLPRT_MRFC(hw->port_info->lport), pf->offset_loaded,
                     &os->mac_remote_faults, &ns->mac_remote_faults);

  ice_stat_update_32(hw, GLPRT_RLEC(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_len_errors, &ns->rx_len_errors);

  ice_stat_update_32(hw, GLPRT_LXONRXC(hw->port_info->lport), pf->offset_loaded,
                     &os->link_xon_rx, &ns->link_xon_rx);
  ice_stat_update_32(hw, GLPRT_LXOFFRXC(hw->port_info->lport),
                     pf->offset_loaded, &os->link_xoff_rx, &ns->link_xoff_rx);
  ice_stat_update_32(hw, GLPRT_LXONTXC(hw->port_info->lport), pf->offset_loaded,
                     &os->link_xon_tx, &ns->link_xon_tx);
  ice_stat_update_32(hw, GLPRT_LXOFFTXC(hw->port_info->lport),
                     pf->offset_loaded, &os->link_xoff_tx, &ns->link_xoff_tx);
  ice_stat_update_40(hw, GLPRT_PRC64H(hw->port_info->lport),
                     GLPRT_PRC64L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_64, &ns->rx_size_64);
  ice_stat_update_40(hw, GLPRT_PRC127H(hw->port_info->lport),
                     GLPRT_PRC127L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_127, &ns->rx_size_127);
  ice_stat_update_40(hw, GLPRT_PRC255H(hw->port_info->lport),
                     GLPRT_PRC255L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_255, &ns->rx_size_255);
  ice_stat_update_40(hw, GLPRT_PRC511H(hw->port_info->lport),
                     GLPRT_PRC511L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_511, &ns->rx_size_511);
  ice_stat_update_40(hw, GLPRT_PRC1023H(hw->port_info->lport),
                     GLPRT_PRC1023L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_1023, &ns->rx_size_1023);
  ice_stat_update_40(hw, GLPRT_PRC1522H(hw->port_info->lport),
                     GLPRT_PRC1522L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_1522, &ns->rx_size_1522);
  ice_stat_update_40(hw, GLPRT_PRC9522H(hw->port_info->lport),
                     GLPRT_PRC9522L(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_size_big, &ns->rx_size_big);
  ice_stat_update_32(hw, GLPRT_RUC(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_undersize, &ns->rx_undersize);
  ice_stat_update_32(hw, GLPRT_RFC(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_fragments, &ns->rx_fragments);
  ice_stat_update_32(hw, GLPRT_ROC(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_oversize, &ns->rx_oversize);
  ice_stat_update_32(hw, GLPRT_RJC(hw->port_info->lport), pf->offset_loaded,
                     &os->rx_jabber, &ns->rx_jabber);
  ice_stat_update_40(hw, GLPRT_PTC64H(hw->port_info->lport),
                     GLPRT_PTC64L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_64, &ns->tx_size_64);
  ice_stat_update_40(hw, GLPRT_PTC127H(hw->port_info->lport),
                     GLPRT_PTC127L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_127, &ns->tx_size_127);
  ice_stat_update_40(hw, GLPRT_PTC255H(hw->port_info->lport),
                     GLPRT_PTC255L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_255, &ns->tx_size_255);
  ice_stat_update_40(hw, GLPRT_PTC511H(hw->port_info->lport),
                     GLPRT_PTC511L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_511, &ns->tx_size_511);
  ice_stat_update_40(hw, GLPRT_PTC1023H(hw->port_info->lport),
                     GLPRT_PTC1023L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_1023, &ns->tx_size_1023);
  ice_stat_update_40(hw, GLPRT_PTC1522H(hw->port_info->lport),
                     GLPRT_PTC1522L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_1522, &ns->tx_size_1522);
  ice_stat_update_40(hw, GLPRT_PTC9522H(hw->port_info->lport),
                     GLPRT_PTC9522L(hw->port_info->lport), pf->offset_loaded,
                     &os->tx_size_big, &ns->tx_size_big);
...
static int ice_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  stats->oerrors = ns->eth.tx_errors + pf->main_vsi->eth_stats.tx_errors;

  /* Rx Errors */
  stats->imissed = ns->eth.rx_discards + pf->main_vsi->eth_stats.rx_discards;
  stats->ierrors = ns->crc_errors + ns->rx_undersize + ns->rx_oversize +
                   ns->rx_fragments + ns->rx_jabber;
  ...
  PMD_DRV_LOG(DEBUG, "illegal_bytes:	%" PRIu64 "", ns->illegal_bytes);
...
static uint32_t ice_xstats_calc_num(void) {
  uint32_t num;
...
static void ice_update_mbuf_stats(struct rte_eth_dev *ethdev,
...
static int ice_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
  ...
  for (i = 0; i < ICE_NB_ETH_XSTATS; i++) {
    xstats[count].value =
        *(uint64_t *)((char *)&hw_stats->eth + ice_stats_strings[i].offset);
  ...
  for (i = 0; i < ICE_NB_HW_PORT_XSTATS; i++) {
    xstats[count].value =
        *(uint64_t *)((char *)hw_stats + ice_hw_port_strings[i].offset);
    xstats[count].id = count;
    count++;
  }
...
static int ice_dev_udp_tunnel_port_add(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    if (!ret && ad->psr != NULL)
      ice_parser_vxlan_tunnel_set(ad->psr, udp_tunnel->udp_port, true);
    break;
  default:
...
static int ice_dev_udp_tunnel_port_del(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    if (!ret && ad->psr != NULL)
      ice_parser_vxlan_tunnel_set(ad->psr, udp_tunnel->udp_port, false);
    break;
  default:
    PMD_DRV_LOG(ERR, "Invalid tunnel type");
    ret = -EINVAL;
...
static int ice_timesync_enable(struct rte_eth_dev *dev) {
  ...
  if (hw->func_caps.ts_func_info.src_tmr_owned) {
    ...
    if (ret) {
      PMD_DRV_LOG(ERR, "Failed to write PHC increment time value");
      return -1;
    }
  }
...
static int ice_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                         struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct ice_adapter),
                                       ice_dev_init);
}

static int ice_pci_remove(struct rte_pci_device *pci_dev) {
...
RTE_PMD_REGISTER_PARAM_STRING(net_ice, ICE_HW_DEBUG_MASK_ARG
                              "=0xXXX" ICE_PROTO_XTR_ARG
                              "=[queue:]<vlan|ipv4|ipv6|ipv6_flow|tcp|ip_"
                              "offset>" ICE_SAFE_MODE_SUPPORT_ARG
                              "=<0|1>" ICE_DEFAULT_MAC_DISABLE
                              "=<0|1>" ICE_RX_LOW_LATENCY_ARG "=<0|1>");
