...
static const char *const iavf_valid_args[] = {IAVF_PROTO_XTR_ARG,
...
static struct iavf_proto_xtr_ol iavf_proto_xtr_params[] = {
    [IAVF_PROTO_XTR_VLAN] = {.param = {.name =
                                           "intel_pmd_dynflag_proto_xtr_vlan"},
                             .ol_flag =
                                 &rte_pmd_ifd_dynflag_proto_xtr_vlan_mask},
    [IAVF_PROTO_XTR_IPV4] = {.param = {.name =
                                           "intel_pmd_dynflag_proto_xtr_ipv4"},
                             .ol_flag =
                                 &rte_pmd_ifd_dynflag_proto_xtr_ipv4_mask},
    [IAVF_PROTO_XTR_IPV6] = {.param = {.name =
                                           "intel_pmd_dynflag_proto_xtr_ipv6"},
                             .ol_flag =
    ...
    [IAVF_PROTO_XTR_TCP] = {.param = {.name =
                                          "intel_pmd_dynflag_proto_xtr_tcp"},
    ...
    [IAVF_PROTO_XTR_IPSEC_CRYPTO_SAID] =
        {.param = {.name = "intel_pmd_dynflag_proto_xtr_ipsec_crypto_said"},
         .ol_flag = &rte_pmd_ifd_dynflag_proto_xtr_ipsec_crypto_said_mask},
};
...
static int iavf_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                    int on);
...
static const struct rte_iavf_xstats_name_off rte_iavf_stats_strings[] = {
    ...
    {"rx_unknown_protocol_packets",
     offsetof(struct iavf_eth_stats, rx_unknown_protocol)},
    ...
    {"inline_ipsec_crypto_ierrors_misc", _OFF_OF(ips_stats.ierrors.misc)},
};
#undef _OFF_OF

#define IAVF_NB_XSTATS                                                         \
  (sizeof(rte_iavf_stats_strings) / sizeof(rte_iavf_stats_strings[0]))
...
static int iavf_tm_ops_get(struct rte_eth_dev *dev, void *arg) {
...
__rte_unused static int iavf_vfr_inprogress(struct iavf_hw *hw) {
  int inprogress = 0;

  if ((IAVF_READ_REG(hw, IAVF_VFGEN_RSTAT) & IAVF_VFGEN_RSTAT_VFR_STATE_MASK) ==
...
__rte_unused static void iavf_dev_watchdog(void *cb_arg) {
  ...
  if (adapter->vf.vf_reset) {
    ...
    if (vfr_inprogress) {
      PMD_DRV_LOG(INFO, "VF \"%s\" reset event detected by watchdog",
                  adapter->vf.eth_dev->data->name);
      ...
      iavf_dev_event_post(adapter->vf.eth_dev, RTE_ETH_EVENT_INTR_RESET, NULL,
                          0);
    }
  }
  ...
  if (adapter->devargs.watchdog_period) {
    /* re-alarm watchdog */
    rc = rte_eal_alarm_set(adapter->devargs.watchdog_period, &iavf_dev_watchdog,
                           cb_arg);
...
static int iavf_set_mc_addr_list(struct rte_eth_dev *dev,
  ...
  err =
      iavf_add_del_mc_addr_list(adapter, vf->mc_addrs, vf->mc_addrs_num, false);
  if (err)
    return err;
  ...
  if (err) {
     ...
     * addresses back.
     */
    ret = iavf_add_del_mc_addr_list(adapter, vf->mc_addrs, vf->mc_addrs_num,
                                    true);
    if (ret)
      return ret;
  } else {
...
static void iavf_config_rss_hf(struct iavf_adapter *adapter, uint64_t rss_hf) {
  static const uint64_t map_hena_rss[] = {
      /* IPv4 */
      [IAVF_FILTER_PCTYPE_NONF_UNICAST_IPV4_UDP] = RTE_ETH_RSS_NONFRAG_IPV4_UDP,
      [IAVF_FILTER_PCTYPE_NONF_MULTICAST_IPV4_UDP] =
          RTE_ETH_RSS_NONFRAG_IPV4_UDP,
      [IAVF_FILTER_PCTYPE_NONF_IPV4_UDP] = RTE_ETH_RSS_NONFRAG_IPV4_UDP,
      [IAVF_FILTER_PCTYPE_NONF_IPV4_TCP_SYN_NO_ACK] =
          RTE_ETH_RSS_NONFRAG_IPV4_TCP,
      [IAVF_FILTER_PCTYPE_NONF_IPV4_TCP] = RTE_ETH_RSS_NONFRAG_IPV4_TCP,
      [IAVF_FILTER_PCTYPE_NONF_IPV4_SCTP] = RTE_ETH_RSS_NONFRAG_IPV4_SCTP,
      [IAVF_FILTER_PCTYPE_NONF_IPV4_OTHER] = RTE_ETH_RSS_NONFRAG_IPV4_OTHER,
      [IAVF_FILTER_PCTYPE_FRAG_IPV4] = RTE_ETH_RSS_FRAG_IPV4,

      /* IPv6 */
      [IAVF_FILTER_PCTYPE_NONF_UNICAST_IPV6_UDP] = RTE_ETH_RSS_NONFRAG_IPV6_UDP,
      [IAVF_FILTER_PCTYPE_NONF_MULTICAST_IPV6_UDP] =
          RTE_ETH_RSS_NONFRAG_IPV6_UDP,
      [IAVF_FILTER_PCTYPE_NONF_IPV6_UDP] = RTE_ETH_RSS_NONFRAG_IPV6_UDP,
      [IAVF_FILTER_PCTYPE_NONF_IPV6_TCP_SYN_NO_ACK] =
          RTE_ETH_RSS_NONFRAG_IPV6_TCP,
      [IAVF_FILTER_PCTYPE_NONF_IPV6_TCP] = RTE_ETH_RSS_NONFRAG_IPV6_TCP,
      [IAVF_FILTER_PCTYPE_NONF_IPV6_SCTP] = RTE_ETH_RSS_NONFRAG_IPV6_SCTP,
      [IAVF_FILTER_PCTYPE_NONF_IPV6_OTHER] = RTE_ETH_RSS_NONFRAG_IPV6_OTHER,
      [IAVF_FILTER_PCTYPE_FRAG_IPV6] = RTE_ETH_RSS_FRAG_IPV6,

      /* L2 Payload */
      [IAVF_FILTER_PCTYPE_L2_PAYLOAD] = RTE_ETH_RSS_L2_PAYLOAD};

  const uint64_t ipv4_rss =
  ...
  if (ret) {
     ...
     */
    PMD_DRV_LOG(WARNING, "fail to get RSS offload type caps, ret: %d", ret);
    return;
  }
  ...
  if (ret) {
     ...
     * for VF, so here just print a warning and return.
     */
    PMD_DRV_LOG(WARNING, "fail to set RSS offload types, ret: %d", ret);
    return;
  }
...
static int iavf_init_rss(struct iavf_adapter *adapter) {
  ...
  nb_q = RTE_MIN(adapter->dev_data->nb_rx_queues, vf->max_rss_qregion);
  ...
  if (!rss_conf->rss_key) {
    ...
    rte_memcpy(vf->rss_key, rss_conf->rss_key,
               RTE_MIN(rss_conf->rss_key_len, vf->vf_res->rss_key_size));
...
static int iavf_queues_req_reset(struct rte_eth_dev *dev, uint16_t num) {
...
static int iavf_dev_vlan_insert_set(struct rte_eth_dev *dev) {
  struct iavf_adapter *adapter =
      IAVF_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  bool enable;
  ...
  enable =
      !!(dev->data->dev_conf.txmode.offloads & RTE_ETH_TX_OFFLOAD_VLAN_INSERT);
  iavf_config_vlan_insert_v2(adapter, enable);

  return 0;
}

static int iavf_dev_init_vlan(struct rte_eth_dev *dev) {
  int err;

  err = iavf_dev_vlan_offload_set(
      dev, RTE_ETH_VLAN_STRIP_MASK | RTE_ETH_QINQ_STRIP_MASK |
               RTE_ETH_VLAN_FILTER_MASK | RTE_ETH_VLAN_EXTEND_MASK);
  if (err) {
    PMD_DRV_LOG(INFO, "VLAN offloading is not supported, or offloading was "
                      "refused by the PF");
    return err;
  }
...
static int iavf_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (num_queue_pairs > IAVF_MAX_NUM_QUEUES_DFLT) {
     ...
     */
    if (vf->lv_enabled || num_queue_pairs > vf->vsi_res->num_queue_pairs) {
      ret = iavf_queues_req_reset(dev, num_queue_pairs);
      if (ret)
        return ret;
...
static int iavf_init_rxq(struct rte_eth_dev *dev, struct iavf_rx_queue *rxq) {
  ...
  max_pkt_len = RTE_MIN((uint32_t)rxq->rx_buf_len * IAVF_MAX_CHAINED_RX_BUFFERS,
                        frame_size);

  /* Check if maximum packet length is set correctly.  */
  if (max_pkt_len <= RTE_ETHER_MIN_LEN || max_pkt_len > IAVF_FRAME_SIZE_MAX) {
    PMD_DRV_LOG(ERR,
                "maximum packet length must be "
                "larger than %u and smaller than %u",
                (uint32_t)IAVF_ETH_MAX_LEN, (uint32_t)IAVF_FRAME_SIZE_MAX);
    return -EINVAL;
  }

  if (rxq->offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP) {
    /* Register mbuf field and flag for Rx timestamp */
    err = rte_mbuf_dyn_rx_timestamp_register(&iavf_timestamp_dynfield_offset,
...
static int iavf_init_queues(struct rte_eth_dev *dev) {
  struct iavf_rx_queue **rxq = (struct iavf_rx_queue **)dev->data->rx_queues;
...
static int iavf_config_rx_queues_irqs(struct rte_eth_dev *dev,
  ...
  if (!dev->data->dev_conf.intr_conf.rxq || !rte_intr_dp_is_en(intr_handle)) {
    ...
    if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_WB_ON_ITR) {
       ...
       */
      IAVF_WRITE_REG(hw, IAVF_VFINT_DYN_CTLN1(vf->msix_base - 1),
      ...
      /* set ITR to default */
      interval = iavf_calc_itr_interval(IAVF_QUEUE_ITR_INTERVAL_DEFAULT);
      IAVF_WRITE_REG(
          hw, IAVF_VFINT_DYN_CTL01,
          IAVF_VFINT_DYN_CTL01_INTENA_MASK |
              (IAVF_ITR_INDEX_DEFAULT << IAVF_VFINT_DYN_CTL01_ITR_INDX_SHIFT) |
              (interval << IAVF_VFINT_DYN_CTL01_INTERVAL_SHIFT));
    }
    IAVF_WRITE_FLUSH(hw);
    ...
    if (!rte_intr_allow_others(intr_handle)) {
      ...
      for (i = 0; i < dev->data->nb_rx_queues; i++) {
        qv_map[i].queue_id = i;
        qv_map[i].vector_id = vf->msix_base;
        rte_intr_vec_list_index_set(intr_handle, i, IAVF_MISC_VEC_ID);
      }
      vf->qv_map = qv_map;
      PMD_DRV_LOG(DEBUG, "vector %u are mapping to all Rx queues",
                  vf->msix_base);
    } else {
      /* If Rx interrupt is required, and we can use
       * multi interrupts, then the vec is from 1
       */
      vf->nb_msix = RTE_MIN(rte_intr_nb_efd_get(intr_handle),
      ...
      vf->qv_map = qv_map;
      PMD_DRV_LOG(DEBUG, "%u vectors are mapping to %u Rx queues", vf->nb_msix,
                  dev->data->nb_rx_queues);
    }
  }
  ...
  if (!vf->lv_enabled) {
    ...
    while (num_qv_maps > IAVF_IRQ_MAP_NUM_PER_BUF) {
      if (iavf_config_irq_map_lv(adapter, IAVF_IRQ_MAP_NUM_PER_BUF, index)) {
...
static int iavf_start_queues(struct rte_eth_dev *dev) {
...
static int iavf_dev_start(struct rte_eth_dev *dev) {
  ...
  while (num_queue_pairs > IAVF_CFG_Q_NUM_PER_BUF) {
    if (iavf_configure_queues(adapter, IAVF_CFG_Q_NUM_PER_BUF, index) != 0) {
...
iavf_dev_supported_ptypes_get(struct rte_eth_dev *dev __rte_unused,
  ...
  static const uint32_t ptypes[] = {
      ...
      RTE_PTYPE_L4_NONFRAG, RTE_PTYPE_L4_SCTP,
...
static int iavf_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  ...
  return iavf_config_promisc(adapter, true, vf->promisc_multicast_enabled);
}
...
static int iavf_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  ...
  return iavf_config_promisc(adapter, false, vf->promisc_multicast_enabled);
}
...
static int iavf_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  ...
  return iavf_config_promisc(adapter, vf->promisc_unicast_enabled, true);
}
...
static int iavf_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  ...
  return iavf_config_promisc(adapter, vf->promisc_unicast_enabled, false);
}
...
static void iavf_iterate_vlan_filters_v2(struct rte_eth_dev *dev, bool enable) {
  ...
  for (i = 0; i < RTE_DIM(vfc->ids); i++) {
    ...
    for (j = 0; ids != 0 && j < 64; j++, ids >>= 1) {
      if (ids & 1)
        iavf_add_del_vlan_v2(adapter, 64 * i + j, enable);
    }
  }
}

static int iavf_dev_vlan_offload_set_v2(struct rte_eth_dev *dev, int mask) {
...
static int iavf_dev_rss_reta_update(struct rte_eth_dev *dev,
  ...
  if (reta_size != vf->vf_res->rss_lut_size) {
    PMD_DRV_LOG(ERR,
                ...
                "support (%d)",
                reta_size, vf->vf_res->rss_lut_size);
    return -EINVAL;
  }
...
static int iavf_set_rss_key(struct iavf_adapter *adapter, uint8_t *key,
                            uint8_t key_len) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  ...
  if (!key || key_len == 0) {
    ...
    PMD_DRV_LOG(ERR,
                "The size of hash key configured "
                "(%d) doesn't match the size of hardware can "
                "support (%d)",
                key_len, vf->vf_res->rss_key_size);
    return -EINVAL;
  }
...
static int iavf_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  /* Set hash key. */
  ret = iavf_set_rss_key(adapter, rss_conf->rss_key, rss_conf->rss_key_len);
  if (ret)
    return ret;
...
static int iavf_dev_mtu_set(struct rte_eth_dev *dev,
...
static int iavf_dev_set_default_mac_addr(struct rte_eth_dev *dev,
  ...
  if (ret)
    PMD_DRV_LOG(ERR, "Fail to delete old MAC:" RTE_ETHER_ADDR_PRT_FMT,
                RTE_ETHER_ADDR_BYTES(old_addr));

  ret = iavf_add_del_eth_addr(adapter, mac_addr, true,
                              VIRTCHNL_ETHER_ADDR_PRIMARY);
  if (ret)
    PMD_DRV_LOG(ERR, "Fail to add new MAC:" RTE_ETHER_ADDR_PRT_FMT,
...
static void iavf_stat_update_48(uint64_t *offset, uint64_t *stat) {
  if (*stat >= *offset)
    *stat = *stat - *offset;
  else
    *stat = (uint64_t)((*stat + ((uint64_t)1 << IAVF_48_BIT_WIDTH)) - *offset);

  *stat &= IAVF_48_BIT_MASK;
}

static void iavf_stat_update_32(uint64_t *offset, uint64_t *stat) {
  if (*stat >= *offset)
    *stat = (uint64_t)(*stat - *offset);
  else
    *stat = (uint64_t)((*stat + ((uint64_t)1 << IAVF_32_BIT_WIDTH)) - *offset);
}

static void iavf_update_stats(struct iavf_vsi *vsi,
...
static int iavf_dev_stats_get(struct rte_eth_dev *dev,
  ...
  if (ret == 0) {
    uint8_t crc_stats_len =
        (dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_KEEP_CRC)
            ? 0
            : RTE_ETHER_CRC_LEN;
    ...
    stats->opackets =
        pstats->tx_broadcast + pstats->tx_multicast + pstats->tx_unicast;
...
static void iavf_dev_update_ipsec_xstats(struct rte_eth_dev *ethdev,
...
static void iavf_dev_update_mbuf_stats(struct rte_eth_dev *ethdev,
...
static int iavf_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < IAVF_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)&iavf_xtats) + rte_iavf_stats_strings[i].offset);
  }
...
static int iavf_dev_rx_queue_intr_enable(struct rte_eth_dev *dev,
  ...
  msix_intr = rte_intr_vec_list_index_get(pci_dev->intr_handle, queue_id);
  if (msix_intr == IAVF_MISC_VEC_ID) {
    ...
    IAVF_WRITE_REG(hw, IAVF_VFINT_DYN_CTLN1(msix_intr - IAVF_RX_VEC_START),
...
static int iavf_check_vf_reset_done(struct iavf_hw *hw) {
  int i, reset;

  for (i = 0; i < IAVF_RESET_WAIT_CNT; i++) {
    reset =
        IAVF_READ_REG(hw, IAVF_VFGEN_RSTAT) & IAVF_VFGEN_RSTAT_VFR_STATE_MASK;
    reset = reset >> IAVF_VFGEN_RSTAT_VFR_STATE_SHIFT;
    if (reset == VIRTCHNL_VFR_VFACTIVE || reset == VIRTCHNL_VFR_COMPLETED)
      break;
    rte_delay_ms(20);
  }
...
static int iavf_lookup_proto_xtr_type(const char *flex_name) {
  static struct {
    const char *name;
 ...
 */
static int iavf_parse_queue_set(const char *input, int xtr_type,
...
static int iavf_parse_queue_proto_xtr(const char *queues,
                                      struct iavf_devargs *devargs) {
...
static int iavf_handle_proto_xtr_arg(__rte_unused const char *key,
  ...
  if (iavf_parse_queue_proto_xtr(value, devargs) < 0) {
    PMD_DRV_LOG(ERR, "the proto_xtr's parameter is wrong : '%s'", value);
    return -1;
  }
...
static int parse_u16(__rte_unused const char *key, const char *value,
                     void *args) {
  u16 *num = (u16 *)args;
  u16 tmp;
  ...
  if (errno || !tmp) {
    PMD_DRV_LOG(WARNING, "%s: \"%s\" is not a valid u16", key, value);
    return -1;
  }
...
static int iavf_parse_watchdog_period(__rte_unused const char *key,
  ...
  tmp = atoi(value);
  if (tmp < 0) {
    PMD_DRV_LOG(WARNING, "%s: \"%s\" is not greater than or equal to zero", key,
                value);
    return -1;
  }
...
static int iavf_parse_mbuf_check(__rte_unused const char *key,
                                 const char *value, void *args) {
  char *cur;
  char *tmp;
  int str_len;
...
static int iavf_parse_devargs(struct rte_eth_dev *dev) {
  ...
  ret = rte_kvargs_process(kvlist, IAVF_QUANTA_SIZE_ARG, &parse_u16,
                           &ad->devargs.quanta_size);
  if (ret)
    goto bail;
  ...
  ret = rte_kvargs_process(kvlist, IAVF_NO_POLL_ON_LINK_DOWN_ARG, &parse_bool,
  ...
  ret = rte_kvargs_process(kvlist, IAVF_MBUF_CHECK_ARG, &iavf_parse_mbuf_check,
                           &ad->devargs.mbuf_check);
  if (ret)
    goto bail;

  ret = rte_kvargs_process(kvlist, IAVF_ENABLE_AUTO_RESET_ARG, &parse_bool,
...
static void iavf_init_proto_xtr(struct rte_eth_dev *dev) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
  struct iavf_adapter *ad = IAVF_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  ...
  vf->proto_xtr = rte_zmalloc("vf proto xtr", vf->vsi_res->num_queue_pairs, 0);
  ...
  for (i = 0; i < vf->vsi_res->num_queue_pairs; i++) {
    vf->proto_xtr[i] = ad->devargs.proto_xtr[i] != IAVF_PROTO_XTR_NONE
  ...
  if (unlikely(offset == -1)) {
    PMD_DRV_LOG(ERR, "failed to extract protocol metadata, error %d",
                -rte_errno);
    return;
  }

  PMD_DRV_LOG(DEBUG, "proto_xtr metadata offset in mbuf is : %d", offset);
  ...
  for (i = 0; i < RTE_DIM(iavf_proto_xtr_params); i++) {
    ...
    if (!(vf->supported_rxdid & BIT(rxdid))) {
      PMD_DRV_LOG(ERR, "rxdid[%u] is not supported in hardware", rxdid);
    ...
    if (unlikely(offset == -1)) {
      PMD_DRV_LOG(ERR, "failed to register proto_xtr offload '%s', error %d",
    ...
    PMD_DRV_LOG(DEBUG, "proto_xtr offload '%s' offset in mbuf is : %d",
                xtr_ol->param.name, offset);
    *xtr_ol->ol_flag = 1ULL << offset;
  }
}

static int iavf_init_vf(struct rte_eth_dev *dev) {
  int err, bufsz;
  struct iavf_adapter *adapter =
      IAVF_DEV_PRIVATE_TO_ADAPTER(dev->data->dev_private);
  struct iavf_hw *hw = IAVF_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) {
    vf->rss_key = rte_zmalloc("rss_key", vf->vf_res->rss_key_size, 0);
    if (!vf->rss_key) {
      PMD_INIT_LOG(ERR, "unable to allocate rss_key memory");
      goto err_rss;
    }
    vf->rss_lut = rte_zmalloc("rss_lut", vf->vf_res->rss_lut_size, 0);
    if (!vf->rss_lut) {
...
static void iavf_uninit_vf(struct rte_eth_dev *dev) {
  struct iavf_hw *hw = IAVF_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static inline void iavf_enable_irq0(struct iavf_hw *hw) {
  /* Enable admin queue interrupt trigger */
  IAVF_WRITE_REG(hw, IAVF_VFINT_ICR0_ENA1, IAVF_VFINT_ICR0_ENA1_ADMINQ_MASK);
...
static inline void iavf_disable_irq0(struct iavf_hw *hw) {
  ...
  IAVF_WRITE_REG(hw, IAVF_VFINT_DYN_CTL01, IAVF_VFINT_DYN_CTL01_ITR_INDX_MASK);
  IAVF_WRITE_FLUSH(hw);
}
...
static void iavf_default_rss_disable(struct iavf_adapter *adapter) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(adapter);
  int ret = 0;
...
static int iavf_dev_init(struct rte_eth_dev *eth_dev) {
  struct iavf_adapter *adapter =
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("iavf_mac", RTE_ETHER_ADDR_LEN * IAVF_NUM_MACADDR_MAX, 0);
  ...
  if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_WB_ON_ITR) {
    ...
    rte_intr_callback_register(pci_dev->intr_handle, iavf_dev_interrupt_handler,
                               (void *)eth_dev);
...
static int iavf_dev_close(struct rte_eth_dev *dev) {
  ...
  if (vf->vf_res->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_WB_ON_ITR) {
    ...
    /* unregister callback func from eal lib */
    rte_intr_callback_unregister(intr_handle, iavf_dev_interrupt_handler, dev);
  } else {
...
static int iavf_dev_uninit(struct rte_eth_dev *dev) {
  struct iavf_info *vf = IAVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static inline bool iavf_is_reset(struct iavf_hw *hw) {
  return !(IAVF_READ_REG(hw, IAVF_VF_ARQLEN1) & IAVF_VF_ARQLEN1_ARQENABLE_MASK);
}
...
static int iavf_dcf_cap_check_handler(__rte_unused const char *key,
                                      const char *value,
...
static int iavf_dcf_cap_selected(struct rte_devargs *devargs) {
  struct rte_kvargs *kvlist;
  const char *key = "cap";
  int ret = 0;
  ...
  if (rte_kvargs_process(kvlist, key, iavf_dcf_cap_check_handler, NULL) < 0)
    goto exit;
...
static int eth_iavf_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct iavf_adapter),
                                       iavf_dev_init);
}
