...
static int fm10k_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int on);
static void fm10k_MAC_filter_set(struct rte_eth_dev *dev, const u8 *mac,
                                 bool add, uint32_t pool);
...
static const struct fm10k_xstats_name_off fm10k_hw_stats_strings[] = {
    ...
    {"rx_mbuf_allocation_errors", offsetof(struct fm10k_hw_stats, nodesc_drop)},
};

#define FM10K_NB_HW_XSTATS                                                     \
  (sizeof(fm10k_hw_stats_strings) / sizeof(fm10k_hw_stats_strings[0]))
...
#define FM10K_NB_RX_Q_XSTATS                                                   \
  (sizeof(fm10k_hw_stats_rx_q_strings) / sizeof(fm10k_hw_stats_rx_q_strings[0]))
...
#define FM10K_NB_TX_Q_XSTATS                                                   \
  (sizeof(fm10k_hw_stats_tx_q_strings) / sizeof(fm10k_hw_stats_tx_q_strings[0]))

#define FM10K_NB_XSTATS                                                        \
  (FM10K_NB_HW_XSTATS +                                                        \
   FM10K_MAX_QUEUES_PF * (FM10K_NB_RX_Q_XSTATS + FM10K_NB_TX_Q_XSTATS))
static int fm10k_dev_rxq_interrupt_setup(struct rte_eth_dev *dev);

static void fm10k_mbx_initlock(struct fm10k_hw *hw) {
  rte_spinlock_init(FM10K_DEV_PRIVATE_TO_MBXLOCK(hw->back));
}

static void fm10k_mbx_lock(struct fm10k_hw *hw)
...
static void fm10k_mbx_unlock(struct fm10k_hw *hw)
...
__rte_weak uint16_t fm10k_recv_pkts_vec(__rte_unused void *rx_queue,
                                        __rte_unused struct rte_mbuf **rx_pkts,
                                        __rte_unused uint16_t nb_pkts) {
  return 0;
}

__rte_weak uint16_t fm10k_recv_scattered_pkts_vec(
    __rte_unused void *rx_queue, __rte_unused struct rte_mbuf **rx_pkts,
    __rte_unused uint16_t nb_pkts) {
  return 0;
}

__rte_weak int fm10k_rxq_vec_setup(__rte_unused struct fm10k_rx_queue *rxq)
...
__rte_weak void
fm10k_rx_queue_release_mbufs_vec(__rte_unused struct fm10k_rx_queue *rxq) {
  return;
}

__rte_weak void fm10k_txq_vec_setup(__rte_unused struct fm10k_tx_queue *txq) {
  return;
}
...
__rte_weak uint16_t fm10k_xmit_fixed_burst_vec(
    __rte_unused void *tx_queue, __rte_unused struct rte_mbuf **tx_pkts,
    __rte_unused uint16_t nb_pkts) {
  return 0;
}
 ...
 */
static inline int rx_queue_reset(struct fm10k_rx_queue *q) {
  ...
  for (i = 0; i < q->nb_desc; ++i) {
    ...
    if (!fm10k_addr_alignment_valid(q->sw_ring[i])) {
      rte_mempool_put_bulk(q->mp, (void **)q->sw_ring, q->nb_desc);
      return -EINVAL;
    }
 ...
 * device.
 */
static inline void rx_queue_clean(struct fm10k_rx_queue *q) {
 ...
 */
static inline void rx_queue_free(struct fm10k_rx_queue *q) {
  PMD_INIT_FUNC_TRACE();
  if (q) {
 ...
 */
static inline int rx_queue_disable(struct fm10k_hw *hw, uint16_t qnum) {
  uint32_t reg, i;

  reg = FM10K_READ_REG(hw, FM10K_RXQCTL(qnum));
  FM10K_WRITE_REG(hw, FM10K_RXQCTL(qnum), reg & ~FM10K_RXQCTL_ENABLE);
 ...
 * device
 */
static inline void tx_queue_reset(struct fm10k_tx_queue *q) {
  PMD_INIT_FUNC_TRACE();
  q->last_free = 0;
  q->next_free = 0;
  q->nb_used = 0;
 ...
 * device
 */
static inline void tx_queue_clean(struct fm10k_tx_queue *q) {
 ...
 * free all queue memory used when releasing the queue (i.e. configure)
 */
static inline void tx_queue_free(struct fm10k_tx_queue *q) {
  PMD_INIT_FUNC_TRACE();
  if (q) {
 ...
 */
static inline int tx_queue_disable(struct fm10k_hw *hw, uint16_t qnum) {
  uint32_t reg, i;

  reg = FM10K_READ_REG(hw, FM10K_TXDCTL(qnum));
  FM10K_WRITE_REG(hw, FM10K_TXDCTL(qnum), reg & ~FM10K_TXDCTL_ENABLE);
...
static int fm10k_check_mq_mode(struct rte_eth_dev *dev) {
  ...
  if (vmdq_conf->nb_queue_pools >
    ...
    PMD_INIT_LOG(ERR, "Too many of queue pools: %d", vmdq_conf->nb_queue_pools);
    return -EINVAL;
  }
...
static int fm10k_dev_configure(struct rte_eth_dev *dev) {
  int ret;
  ...
  if (ret != 0) {
    PMD_DRV_LOG(ERR, "fm10k_check_mq_mode fails with %d.", ret);
    return ret;
  }
...
static void fm10k_dev_vmdq_rx_configure(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_eth_vmdq_rx_conf *vmdq_conf;
  uint32_t i;
...
static void fm10k_dev_pf_main_vsi_reset(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static void fm10k_dev_rss_configure(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
#define RSS_KEY_SIZE 40
  ...
  for (i = 0, j = 0; i < FM10K_MAX_RSS_INDICES; i++, j++) {
    ...
    if ((i & 3) == 3)
      FM10K_WRITE_REG(hw, FM10K_RETA(0, i >> 2), rte_bswap32(reta));
  }
  ...
  if (mrqc == 0) {
    PMD_INIT_LOG(ERR,
                 "Specified RSS mode 0x%" PRIx64 "is not"
                 "supported",
                 hf);
    return;
  }
...
static void fm10k_dev_logic_port_update(struct rte_eth_dev *dev,
...
static void fm10k_dev_mq_rx_configure(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_eth_vmdq_rx_conf *vmdq_conf;
  struct rte_eth_conf *dev_conf = &dev->data->dev_conf;
  ...
  memset(dev->data->mac_addrs, 0, RTE_ETHER_ADDR_LEN * FM10K_MAX_MACADDR_NUM);
...
static int fm10k_dev_tx_init(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int i, ret;
  ...
  for (i = 0; i < hw->mac.max_queues; i++)
    FM10K_WRITE_REG(hw, FM10K_TXINT(i), 3 << FM10K_TXINT_TIMER_SHIFT);
  ...
  for (i = 0; i < dev->data->nb_tx_queues; ++i) {
    ...
    FM10K_WRITE_REG(hw, FM10K_TDBAL(i), base_addr & UINT64_LOWER_32BITS_MASK);
...
static int fm10k_dev_rx_init(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct fm10k_macvlan_filter_info *macvlan;
  ...
  for (; i < hw->mac.max_queues; i++)
    FM10K_WRITE_REG(hw, FM10K_RXINT(i), 3 << FM10K_RXINT_TIMER_SHIFT);
  ...
  for (i = 0; i < dev->data->nb_rx_queues; ++i) {
    ...
    FM10K_WRITE_REG(hw, FM10K_RDBAL(i), base_addr & UINT64_LOWER_32BITS_MASK);
    ...
    buf_size =
        (uint16_t)(rte_pktmbuf_data_room_size(rxq->mp) - RTE_PKTMBUF_HEADROOM);
...
static int fm10k_dev_rx_queue_start(struct rte_eth_dev *dev,
  ...
  if (err == -ENOMEM) {
    ...
    PMD_INIT_LOG(ERR,
                 "Invalid buffer address alignment :"
                 " %d",
                 err);
    return err;
  }
...
static int fm10k_dev_rx_queue_stop(struct rte_eth_dev *dev,
                                   uint16_t rx_queue_id) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_dev_tx_queue_start(struct rte_eth_dev *dev,
...
#define FM10K_TXDCTL_WRITE_BACK_MIN_DELAY 0x00010000
  ...
  /* enable TX queue */
  FM10K_WRITE_REG(hw, FM10K_TXDCTL(tx_queue_id), FM10K_TXDCTL_ENABLE | txdctl);
  FM10K_WRITE_FLUSH(hw);
...
static int fm10k_dev_tx_queue_stop(struct rte_eth_dev *dev,
                                   uint16_t tx_queue_id) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static inline int fm10k_glort_valid(struct fm10k_hw *hw) {
  return ((hw->mac.dglort_map & FM10K_DGLORTMAP_NONE) != FM10K_DGLORTMAP_NONE);
}
...
static int fm10k_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  ...
  fm10k_mbx_lock(hw);
  status = hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, mode);
  fm10k_mbx_unlock(hw);
...
static void fm10k_dev_dglort_map_configure(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  dglortmask =
      (GLORT_PF_MASK << FM10K_DGLORTMAP_MASK_SHIFT) | hw->mac.dglort_map;
...
static int fm10k_dev_start(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  int i, diag;
...
static int fm10k_dev_stop(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pdev->intr_handle;
  int i;
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    for (i = 0; i < dev->data->nb_rx_queues; i++) {
      FM10K_WRITE_REG(hw, FM10K_RXINT(i), 3 << FM10K_RXINT_TIMER_SHIFT);
      if (hw->mac.type == fm10k_mac_pf)
        FM10K_WRITE_REG(hw, FM10K_ITR(Q2V(pdev, i)), FM10K_ITR_MASK_SET);
      else
        FM10K_WRITE_REG(hw, FM10K_VFITR(Q2V(pdev, i)), FM10K_ITR_MASK_SET);
    }
  }
...
static void fm10k_dev_queue_release(struct rte_eth_dev *dev) {
  int i;
    ...
    for (q = 0; q < FM10K_MAX_QUEUES_PF; q++) {
      for (i = 0; i < FM10K_NB_RX_Q_XSTATS; i++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "rx_q%u_%s", q, fm10k_hw_stats_rx_q_strings[i].name);
        count++;
      }
      for (i = 0; i < FM10K_NB_TX_Q_XSTATS; i++) {
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "tx_q%u_%s", q, fm10k_hw_stats_tx_q_strings[i].name);
        count++;
      }
    }
  }
...
static int fm10k_xstats_get(struct rte_eth_dev *dev,
...
static int fm10k_stats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; (i < RTE_ETHDEV_QUEUE_STAT_CNTRS) && (i < hw->mac.max_queues);
       ++i) {
...
static int fm10k_stats_reset(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct fm10k_hw_stats *hw_stats =
      FM10K_DEV_PRIVATE_TO_STATS(dev->data->dev_private);
...
static int fm10k_dev_infos_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      fm10k_get_rx_port_offloads_capa(dev) | dev_info->rx_queue_offload_capa;
  dev_info->tx_queue_offload_capa = fm10k_get_tx_queue_offloads_capa(dev);
  dev_info->tx_offload_capa =
      fm10k_get_tx_port_offloads_capa(dev) | dev_info->tx_queue_offload_capa;

  dev_info->hash_key_size = FM10K_RSSRK_SIZE * sizeof(uint32_t);
  dev_info->reta_size = FM10K_MAX_RSS_INDICES;
  dev_info->flow_type_rss_offloads =
      RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_IPV6_EX |
...
static const uint32_t *fm10k_dev_supported_ptypes_get(struct rte_eth_dev *dev,
                                                      size_t *no_of_elements) {
  if (dev->rx_pkt_burst == fm10k_recv_pkts ||
      dev->rx_pkt_burst == fm10k_recv_scattered_pkts) {
    static uint32_t ptypes[] = {
        /* refers to rx_desc_to_ol_flags() */
        RTE_PTYPE_L2_ETHER, RTE_PTYPE_L3_IPV4,     RTE_PTYPE_L3_IPV4_EXT,
    ...
    *no_of_elements = RTE_DIM(ptypes);
    ...
    static uint32_t ptypes_vec[] = {
        ...
        RTE_PTYPE_L3_IPV6_EXT,   RTE_PTYPE_L4_TCP,       RTE_PTYPE_L4_UDP,
        RTE_PTYPE_TUNNEL_GENEVE, RTE_PTYPE_TUNNEL_NVGRE, RTE_PTYPE_TUNNEL_VXLAN,
...
static int fm10k_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
  ...
  for (mac_index = 0;
       (mac_index < FM10K_MAX_MACADDR_NUM) && (result == FM10K_SUCCESS);
       mac_index++) {
...
static int fm10k_vlan_offload_set(struct rte_eth_dev *dev __rte_unused,
...
static void fm10k_MAC_filter_set_vmdq(struct rte_eth_dev *dev, const u8 *mac,
                                      bool add, uint32_t pool) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct fm10k_macvlan_filter_info *macvlan;
  struct rte_eth_vmdq_rx_conf *vmdq_conf;
...
static int fm10k_macaddr_add(struct rte_eth_dev *dev,
...
static void fm10k_macaddr_remove(struct rte_eth_dev *dev, uint32_t index) {
  struct rte_eth_dev_data *data = dev->data;
  struct fm10k_macvlan_filter_info *macvlan;

  macvlan = FM10K_DEV_PRIVATE_TO_MACVLAN(dev->data->dev_private);
  fm10k_MAC_filter_set(dev, data->mac_addrs[index].addr_bytes, FALSE,
                       macvlan->mac_vmdq_id[index]);
  macvlan->mac_vmdq_id[index] = 0;
}

static inline int check_nb_desc(uint16_t min, uint16_t max, uint16_t mult,
                                uint16_t request) {
...
static inline int check_thresh(uint16_t min, uint16_t max, uint16_t div,
                               uint16_t request) {
...
static inline int handle_rxconf(struct fm10k_rx_queue *q,
  ...
  if (check_thresh(FM10K_RX_FREE_THRESH_MIN(q), FM10K_RX_FREE_THRESH_MAX(q),
                   FM10K_RX_FREE_THRESH_DIV(q), rx_free_thresh)) {
    PMD_INIT_LOG(ERR,
                 "rx_free_thresh (%u) must be "
                 "less than or equal to %u, "
                 "greater than or equal to %u, "
                 "and a divisor of %u",
                 rx_free_thresh, FM10K_RX_FREE_THRESH_MAX(q),
                 FM10K_RX_FREE_THRESH_MIN(q), FM10K_RX_FREE_THRESH_DIV(q));
    return -EINVAL;
  }
 ...
 */
static int mempool_element_size_valid(struct rte_mempool *mp) {
  uint32_t min_size;

  /* elt_size includes mbuf header and headroom */
  min_size = mp->elt_size - sizeof(struct rte_mbuf) - RTE_PKTMBUF_HEADROOM;
...
static uint64_t fm10k_get_rx_port_offloads_capa(struct rte_eth_dev *dev) {
  ...
  return (
      uint64_t)(RTE_ETH_RX_OFFLOAD_VLAN_STRIP | RTE_ETH_RX_OFFLOAD_VLAN_FILTER |
                RTE_ETH_RX_OFFLOAD_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM |
                RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_RSS_HASH);
}

static int fm10k_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_id,
  ...
  if (check_nb_desc(FM10K_MIN_RX_DESC, FM10K_MAX_RX_DESC, FM10K_MULT_RX_DESC,
                    nb_desc)) {
    PMD_INIT_LOG(ERR,
                 "Number of Rx descriptors (%u) must be "
  ...
  q->queue_id = queue_id;
  q->tail_ptr =
      (volatile uint32_t *)&((uint32_t *)hw->hw_addr)[FM10K_RDT(queue_id)];
  q->offloads = offloads;
  ...
  q->sw_ring = rte_zmalloc_socket(
      "fm10k sw ring", (nb_desc + q->nb_fake_desc) * sizeof(struct rte_mbuf *),
   ...
   */
  mz = rte_eth_dma_zone_reserve(dev, "rx_ring", queue_id, FM10K_MAX_RX_RING_SZ,
                                FM10K_ALIGN_RX_DESC, socket_id);
  if (mz == NULL) {
...
static inline int handle_txconf(struct fm10k_tx_queue *q,
  ...
  /* make sure the requested threshold satisfies the constraints */
  if (check_thresh(FM10K_TX_FREE_THRESH_MIN(q), FM10K_TX_FREE_THRESH_MAX(q),
                   FM10K_TX_FREE_THRESH_DIV(q), tx_free_thresh)) {
    PMD_INIT_LOG(ERR,
                 "tx_free_thresh (%u) must be "
                 "less than or equal to %u, "
                 "greater than or equal to %u, "
                 "and a divisor of %u",
                 tx_free_thresh, FM10K_TX_FREE_THRESH_MAX(q),
                 FM10K_TX_FREE_THRESH_MIN(q), FM10K_TX_FREE_THRESH_DIV(q));
    return -EINVAL;
  }
  ...
  /* make sure the requested threshold satisfies the constraints */
  if (check_thresh(FM10K_TX_RS_THRESH_MIN(q), FM10K_TX_RS_THRESH_MAX(q),
                   FM10K_TX_RS_THRESH_DIV(q), tx_rs_thresh)) {
    PMD_INIT_LOG(ERR,
                 "tx_rs_thresh (%u) must be "
                 "less than or equal to %u, "
                 "greater than or equal to %u, "
                 "and a divisor of %u",
                 tx_rs_thresh, FM10K_TX_RS_THRESH_MAX(q),
                 FM10K_TX_RS_THRESH_MIN(q), FM10K_TX_RS_THRESH_DIV(q));
    return -EINVAL;
  }
...
static uint64_t fm10k_get_tx_port_offloads_capa(struct rte_eth_dev *dev) {
  ...
  return (
      uint64_t)(RTE_ETH_TX_OFFLOAD_VLAN_INSERT | RTE_ETH_TX_OFFLOAD_MULTI_SEGS |
                RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM |
                RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_TSO);
}

static int fm10k_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_id,
                                uint16_t nb_desc, unsigned int socket_id,
                                const struct rte_eth_txconf *conf) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  /* make sure a valid number of descriptors have been requested */
  if (check_nb_desc(FM10K_MIN_TX_DESC, FM10K_MAX_TX_DESC, FM10K_MULT_TX_DESC,
                    nb_desc)) {
    PMD_INIT_LOG(ERR,
                 "Number of Tx descriptors (%u) must be "
                 "less than or equal to %" PRIu32 ", "
                 "greater than or equal to %u, "
                 "and a multiple of %u",
  ...
  q->ops = &def_txq_ops;
  q->tail_ptr =
      (volatile uint32_t *)&((uint32_t *)hw->hw_addr)[FM10K_TDT(queue_id)];
  ...
  /* allocate memory for the software ring */
  q->sw_ring =
      rte_zmalloc_socket("fm10k sw ring", nb_desc * sizeof(struct rte_mbuf *),
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (q->sw_ring == NULL) {
    PMD_INIT_LOG(ERR, "Cannot allocate software ring");
    rte_free(q);
   ...
   * resizing in later calls to the queue setup function.
   */
  mz = rte_eth_dma_zone_reserve(dev, "tx_ring", queue_id, FM10K_MAX_TX_RING_SZ,
                                FM10K_ALIGN_TX_DESC, socket_id);
  if (mz == NULL) {
    PMD_INIT_LOG(ERR, "Cannot allocate hardware ring");
    rte_free(q->sw_ring);
    rte_free(q);
   ...
   */
  q->rs_tracker.list = rte_zmalloc_socket(
      "fm10k rs tracker", ((nb_desc + 1) / q->rs_thresh) * sizeof(uint16_t),
      RTE_CACHE_LINE_SIZE, socket_id);
...
static int fm10k_reta_update(struct rte_eth_dev *dev,
  ...
  if (reta_size > FM10K_MAX_RSS_INDICES) {
    PMD_INIT_LOG(ERR,
                 ...
                 "(%d)",
                 reta_size, FM10K_MAX_RSS_INDICES);
    return -EINVAL;
  }
  ...
  for (i = 0; i < FM10K_MAX_RSS_INDICES; i += CHARS_PER_UINT32) {
    ...
    mask = (uint8_t)((reta_conf[idx].mask >> shift) & BIT_MASK_PER_UINT32);
    if (mask == 0)
      continue;
...
static int fm10k_reta_query(struct rte_eth_dev *dev,
                            struct rte_eth_rss_reta_entry64 *reta_conf,
                            uint16_t reta_size) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint16_t i, j, idx, shift;
  ...
  for (i = 0; i < FM10K_MAX_RSS_INDICES; i += CHARS_PER_UINT32) {
    ...
    for (j = 0; j < CHARS_PER_UINT32; j++) {
      if (mask & (0x1 << j))
        reta_conf[idx].reta[shift + j] = ((reta >> CHAR_BIT * j) & UINT8_MAX);
    }
  }
...
static int fm10k_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (key &&
      (rss_conf->rss_key_len < FM10K_RSSRK_SIZE * FM10K_RSSRK_ENTRIES_PER_REG))
    return -EINVAL;
...
static int fm10k_rss_hash_conf_get(struct rte_eth_dev *dev,
                                   struct rte_eth_rss_conf *rss_conf) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t *key = (uint32_t *)rss_conf->rss_key;
  uint32_t mrqc;
...
static void fm10k_dev_enable_intr_pf(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  FM10K_WRITE_REG(
      hw, FM10K_EIMR,
      FM10K_EIMR_ENABLE(PCA_FAULT) | FM10K_EIMR_ENABLE(THI_FAULT) |
          FM10K_EIMR_ENABLE(FUM_FAULT) | FM10K_EIMR_ENABLE(MAILBOX) |
          FM10K_EIMR_ENABLE(SWITCHREADY) | FM10K_EIMR_ENABLE(SWITCHNOTREADY) |
          FM10K_EIMR_ENABLE(SRAMERROR) | FM10K_EIMR_ENABLE(VFLR));

  /* Enable ITR 0 */
  FM10K_WRITE_REG(hw, FM10K_ITR(0), FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
  FM10K_WRITE_FLUSH(hw);
}

static void fm10k_dev_disable_intr_pf(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  FM10K_WRITE_REG(
      hw, FM10K_EIMR,
      FM10K_EIMR_DISABLE(PCA_FAULT) | FM10K_EIMR_DISABLE(THI_FAULT) |
          FM10K_EIMR_DISABLE(FUM_FAULT) | FM10K_EIMR_DISABLE(MAILBOX) |
          FM10K_EIMR_DISABLE(SWITCHREADY) | FM10K_EIMR_DISABLE(SWITCHNOTREADY) |
          FM10K_EIMR_DISABLE(SRAMERROR) | FM10K_EIMR_DISABLE(VFLR));
...
static void fm10k_dev_enable_intr_vf(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t int_map = FM10K_INT_MAP_IMMEDIATE;
  ...
  /* Enable ITR 0 */
  FM10K_WRITE_REG(hw, FM10K_VFITR(0),
                  FM10K_ITR_AUTOMASK | FM10K_ITR_MASK_CLEAR);
  FM10K_WRITE_FLUSH(hw);
}

static void fm10k_dev_disable_intr_vf(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  uint32_t int_map = FM10K_INT_MAP_DISABLE;
...
static int fm10k_dev_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                          uint16_t queue_id) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
...
static int fm10k_dev_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                           uint16_t queue_id) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
  ...
  /* Disable ITR */
  if (hw->mac.type == fm10k_mac_pf)
    FM10K_WRITE_REG(hw, FM10K_ITR(Q2V(pdev, queue_id)), FM10K_ITR_MASK_SET);
  else
    FM10K_WRITE_REG(hw, FM10K_VFITR(Q2V(pdev, queue_id)), FM10K_ITR_MASK_SET);
  return 0;
}
  ...
  if (rte_intr_dp_is_en(intr_handle) && !result) {
    if (!rte_intr_vec_list_alloc(intr_handle, "intr_vec",
      ...
      for (queue_id = 0, vec = FM10K_RX_VEC_START;
           queue_id < dev->data->nb_rx_queues; queue_id++) {
        rte_intr_vec_list_index_set(intr_handle, queue_id, vec);
        int nb_efd = rte_intr_nb_efd_get(intr_handle);
        if (vec < (uint32_t)nb_efd - 1 + FM10K_RX_VEC_START)
          vec++;
      }
    } else {
      PMD_INIT_LOG(ERR,
...
static int fm10k_dev_handle_fault(struct fm10k_hw *hw, uint32_t eicr) {
  ...
  if (eicr & FM10K_EICR_PCA_FAULT) {
    ...
    case PCA_UNMAPPED_ADDR:
      estr = "PCA_UNMAPPED_ADDR";
      break;
    case PCA_BAD_QACCESS_PF:
      estr = "PCA_BAD_QACCESS_PF";
      break;
    case PCA_BAD_QACCESS_VF:
      estr = "PCA_BAD_QACCESS_VF";
      break;
    case PCA_MALICIOUS_REQ:
      estr = "PCA_MALICIOUS_REQ";
      break;
    case PCA_POISONED_TLP:
      estr = "PCA_POISONED_TLP";
      break;
    case PCA_TLP_ABORT:
      estr = "PCA_TLP_ABORT";
      break;
    ...
    PMD_INIT_LOG(ERR, "%s: %s(%d) Addr:0x%" PRIx64 " Spec: 0x%x", estr,
                 fault.func ? "VF" : "PF", fault.func, fault.address,
                 fault.specinfo);
  }
  ...
  if (eicr & FM10K_EICR_FUM_FAULT) {
    ...
    case FUM_UNMAPPED_ADDR:
      estr = "FUM_UNMAPPED_ADDR";
      break;
    case FUM_POISONED_TLP:
      estr = "FUM_POISONED_TLP";
      break;
    case FUM_BAD_VF_QACCESS:
      estr = "FUM_BAD_VF_QACCESS";
      break;
    case FUM_ADD_DECODE_ERR:
      estr = "FUM_ADD_DECODE_ERR";
      break;
    case FUM_RO_ERROR:
      estr = "FUM_RO_ERROR";
      break;
    case FUM_QPRC_CRC_ERROR:
      estr = "FUM_QPRC_CRC_ERROR";
      break;
    case FUM_CSR_TIMEOUT:
      estr = "FUM_CSR_TIMEOUT";
      break;
    case FUM_INVALID_TYPE:
      estr = "FUM_INVALID_TYPE";
      break;
    case FUM_INVALID_LENGTH:
      estr = "FUM_INVALID_LENGTH";
      break;
    case FUM_INVALID_BE:
      estr = "FUM_INVALID_BE";
      break;
    case FUM_INVALID_ALIGN:
      estr = "FUM_INVALID_ALIGN";
      break;
    default:
      goto error;
    }
 ...
 *  void
 */
static void fm10k_dev_interrupt_handler_pf(void *param) {
  ...
  if (cause & FM10K_EICR_SWITCHREADY) {
    ...
    if (dev_info->sm_down == 1) {
      ...
      status_mbx = hw->mac.ops.update_lport_state(hw, hw->mac.dglort_map,
                                                  MAX_LPORT_NUM, 1);
  ...
  cause &=
      FM10K_EICR_SWITCHNOTREADY | FM10K_EICR_MAILBOX | FM10K_EICR_SWITCHREADY;
  if (cause)
 ...
 *  void
 */
static void fm10k_dev_interrupt_handler_vf(void *param) {
  struct rte_eth_dev *dev = (struct rte_eth_dev *)param;
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int fm10k_setup_mbx_service(struct fm10k_hw *hw) {
  int err = 0;
  ...
  if (err) {
    PMD_INIT_LOG(ERR, "Failed to register mailbox handler.err:%d", err);
    return err;
  }
...
static int fm10k_dev_close(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pdev->intr_handle;
  int ret;
  ...
  fm10k_mbx_lock(hw);
  hw->mac.ops.update_lport_state(hw, hw->mac.dglort_map, MAX_LPORT_NUM, false);
  fm10k_mbx_unlock(hw);
  ...
  if (hw->mac.type == fm10k_mac_pf) {
    ...
    rte_intr_callback_unregister(intr_handle, fm10k_dev_interrupt_handler_pf,
                                 (void *)dev);
  } else {
    /* disable interrupt */
    fm10k_dev_disable_intr_vf(dev);

    rte_intr_callback_unregister(intr_handle, fm10k_dev_interrupt_handler_vf,
                                 (void *)dev);
  }
...
static int ftag_check_handler(__rte_unused const char *key, const char *value,
...
static int fm10k_check_ftag(struct rte_devargs *devargs) {
  ...
  if (rte_kvargs_process(kvlist, ftag_key, ftag_check_handler, NULL) < 0) {
    rte_kvargs_free(kvlist);
    return 0;
  }
  rte_kvargs_free(kvlist);
...
static uint16_t fm10k_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  uint16_t nb_tx = 0;
  ...
  while (nb_pkts) {
    ...
    ret = fm10k_xmit_fixed_burst_vec(tx_queue, &tx_pkts[nb_tx], num);
    nb_tx += ret;
    nb_pkts -= ret;
    if (ret < num)
      break;
...
static void __rte_cold fm10k_set_tx_function(struct rte_eth_dev *dev) {
  struct fm10k_tx_queue *txq;
  int i;
  int use_sse = 1;
...
static void __rte_cold fm10k_set_rx_function(struct rte_eth_dev *dev) {
  struct fm10k_dev_info *dev_info =
      FM10K_DEV_PRIVATE_TO_INFO(dev->data->dev_private);
  ...
  if (!fm10k_rx_vec_condition_check(dev) && dev_info->rx_vec_allowed &&
      !rx_ftag_en && rte_vect_get_max_simd_bitwidth() >= RTE_VECT_SIMD_128) {
  ...
  rx_using_sse = (dev->rx_pkt_burst == fm10k_recv_scattered_pkts_vec ||
                  dev->rx_pkt_burst == fm10k_recv_pkts_vec);
...
static void fm10k_params_init(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int eth_fm10k_dev_init(struct rte_eth_dev *dev) {
  struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  struct rte_pci_device *pdev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pdev->intr_handle;
  int diag, i, ret;
  ...
  dev->data->mac_addrs =
      rte_zmalloc("fm10k", RTE_ETHER_ADDR_LEN * FM10K_MAX_MACADDR_NUM, 0);
  ...
  if (hw->mac.type == fm10k_mac_pf) {
    ...
    rte_intr_callback_register(intr_handle, fm10k_dev_interrupt_handler_pf,
                               (void *)dev);

    /* enable MISC interrupt */
    fm10k_dev_enable_intr_pf(dev);
  } else { /* VF */
    rte_intr_callback_register(intr_handle, fm10k_dev_interrupt_handler_vf,
                               (void *)dev);
   ...
   * API func.
   */
  hw->mac.ops.update_xcast_mode(hw, hw->mac.dglort_map, FM10K_XCAST_MODE_NONE);
...
static int eth_fm10k_dev_uninit(struct rte_eth_dev *dev) {
  PMD_INIT_FUNC_TRACE();
  fm10k_dev_close(dev);
  return 0;
}
...
static int eth_fm10k_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct fm10k_adapter),
                                       eth_fm10k_dev_init);
}
